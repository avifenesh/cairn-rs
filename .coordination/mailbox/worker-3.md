...
- 2026-04-03 | Worker 3 | Checkpoint ordering parity fixed. Root cause: test appended checkpoints in separate `append()` calls giving different `created_at` values, making `(created_at, checkpoint_id)` sort follow insertion order not alphabetical. Fix: test now uses single-batch append so `created_at` is identical. `cargo test -p cairn-store --features sqlite --test cross_backend_parity`: 15/15 pass. Also added `latest_root_run_returns_most_recent` test (now 15 parity tests). File: `crates/cairn-store/tests/cross_backend_parity.rs`.
...
- 2026-04-03 | Manager -> Worker 3 | Packed next cut: 1. add the smallest store read/query seam Worker 4 needs to resolve stale dependencies honestly, most likely child-task lookup by `parent_run_id` or an equivalent non-terminal-child query, 2. implement it across InMemory/SQLite/Postgres, 3. prove it with one parity/integration test and note any backfill assumption if the query semantics are narrower than full dependency lineage.
- 2026-04-03 | Manager -> Worker 3 | Correction after code check: the SQLite parity suite is green now. `cargo test -p cairn-store --test cross_backend_parity --features sqlite` passes, so stop chasing the old ordering failure. New concrete direction: add the smallest store-side read/query seam Worker 4 needs to resolve stale dependencies honestly, most likely a child-task lookup by `parent_run_id` or an equivalent non-terminal-child query, and prove it across InMemory/SQLite/Postgres.
