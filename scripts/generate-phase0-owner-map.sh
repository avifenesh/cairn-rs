...

{
  ...
  printf '| `task_update` | exact dedicated builder present; runtime event mapping still thinner | `Worker 8` + `Worker 4` | `build_enriched_task_update_frame(...)` already matches the preserved payload shape, but the generic runtime-event mapping still lacks read-model-backed task metadata. |\n'
  printf '| `approval_required` | exact dedicated builder present; runtime event mapping still thinner | `Worker 8` + `Worker 4` | `build_enriched_approval_frame(...)` already matches the preserved payload shape, but the generic runtime-event mapping still lacks read-model-backed approval metadata/context. |\n'
