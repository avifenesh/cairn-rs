...
while true; do
  ...
  ORCH_RESP=$(curl -s -w '\n%{http_code}' -X POST -H "Authorization: Bearer ${TOKEN}" -H "Content-Type: application/json" \
    -d "{\"goal\":\"Use the shell_exec tool to run 'gh issue view ${ISSUE_NUM} --repo ${DOGFOOD_REPO}' and read the issue. Summarize what it asks for in under 50 words.\",\"model_id\":\"${DOGFOOD_MODEL}\",\"max_iterations\":3,\"timeout_ms\":60000}" \
