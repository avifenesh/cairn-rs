...
fn local()      -> EntitlementSet { EntitlementSet::new(tenant(), ProductTier::LocalEval) }
fn team()       -> EntitlementSet { EntitlementSet::new(tenant(), ProductTier::TeamSelfHosted) }
fn enterprise() -> EntitlementSet { EntitlementSet::new(tenant(), ProductTier::EnterpriseSelfHosted) }
...
fn product_tier_serde_round_trip() {
    for tier in [ProductTier::LocalEval, ProductTier::TeamSelfHosted, ProductTier::EnterpriseSelfHosted] {
...
fn product_tier_serializes_to_snake_case() {
    assert_eq!(serde_json::to_string(&ProductTier::LocalEval).unwrap(),           r#""local_eval""#);
    assert_eq!(serde_json::to_string(&ProductTier::TeamSelfHosted).unwrap(),      r#""team_self_hosted""#);
    assert_eq!(serde_json::to_string(&ProductTier::EnterpriseSelfHosted).unwrap(),r#""enterprise_self_hosted""#);
}
...
fn product_tier_variants_are_distinct() {
    ...
    assert_ne!(ProductTier::TeamSelfHosted, ProductTier::EnterpriseSelfHosted);
...
fn entitlement_and_feature_flag_serde_round_trips() {
    ...
    for flag in [FeatureFlag::Preview, FeatureFlag::GeneralAvailability, FeatureFlag::EntitlementGated] {
...
fn is_enterprise_only_true_for_enterprise_tier() {
    ...
    assert!(enterprise().is_enterprise(),  "EnterpriseSelfHosted must be enterprise");
}
...
fn with_entitlement_is_builder_style_accumulation() {
    ...
    for ent in [Entitlement::GovernanceCompliance, Entitlement::AdvancedAdmin,
                Entitlement::ManagedServiceRights, Entitlement::DeploymentTier] {
        assert!(full.has(ent), "chained with_entitlement must add {ent:?}");
    }
    assert_eq!(full.active.len(), 4, "four distinct entitlements must be present");
}
...
fn with_entitlement_deduplicates() {
    ...
    assert_eq!(set.active.len(), 1, "duplicate entitlements must be deduplicated");
    assert!(set.has(Entitlement::GovernanceCompliance));
}
...
fn local_eval_tier_ga_allowed_gated_denied() {
    ...
    assert_eq!(gate.check(&set, "retrieval_core"),  FeatureGateResult::Allowed);

    assert!(matches!(gate.check(&set, "advanced_audit_export"), FeatureGateResult::Denied { .. }));
    assert!(matches!(gate.check(&set, "advanced_admin"),        FeatureGateResult::Denied { .. }));
}
...
fn team_tier_with_governance_unlocks_compliance_features() {
    ...
    assert_eq!(gate.check(&set, "advanced_audit_export"),   FeatureGateResult::Allowed);
    assert_eq!(gate.check(&set, "compliance_policy_packs"),  FeatureGateResult::Allowed);
    assert_eq!(gate.check(&set, "approval_hardening"),       FeatureGateResult::Allowed);
...
fn enterprise_tier_with_all_entitlements_unlocks_everything() {
    ...
    for feature in &["runtime_core", "retrieval_core", "advanced_audit_export",
                     "compliance_policy_packs", "approval_hardening", "advanced_admin"] {
        assert_eq!(
    ...
    for set in [local(), team(), enterprise()] {
        for feature in &["runtime_core", "retrieval_core"] {
            assert_eq!(
                gate.check(&set, feature),
                FeatureGateResult::Allowed,
                "GA feature '{feature}' must pass for tier {:?}", set.tier
            );
        }
    }
}
...
fn feature_flag_ga_always_allowed() {
    ...
    for set in [local(), team(), enterprise()] {
        assert_eq!(gate.check(&set, "test_ga"), FeatureGateResult::Allowed,
            "GA feature must be Allowed for tier {:?}", set.tier);
    }
}
...
fn feature_flag_preview_always_allowed() {
    ...
    for set in [local(), team(), enterprise()] {
        assert_eq!(gate.check(&set, "test_preview"), FeatureGateResult::Allowed,
            "Preview feature must be Allowed for tier {:?}", set.tier);
    }
}
...
fn feature_flag_entitlement_gated_denied_and_allowed() {
    ...
    assert!(
        matches!(gate.check(&without, "test_gated"), FeatureGateResult::Denied { .. }),
...
fn entitlement_gated_with_no_required_entitlement_is_always_allowed() {
    ...
    assert_eq!(gate.check(&local(), "test_open_gate"), FeatureGateResult::Allowed);
}
...
fn capability_mapping_covers_all_entitlement_categories() {
    ...
    for feature in &["advanced_audit_export", "compliance_policy_packs", "approval_hardening"] {
        ...
        assert_eq!(gate.check(&with_gov, feature), FeatureGateResult::Allowed,
            "'{feature}' must be unlocked by GovernanceCompliance");
    }

    // AdvancedAdmin gates one feature.
    let bare = enterprise();
    assert!(matches!(gate.check(&bare, "advanced_admin"), FeatureGateResult::Denied { .. }));
    let with_adm = enterprise().with_entitlement(Entitlement::AdvancedAdmin);
    assert_eq!(gate.check(&with_adm, "advanced_admin"), FeatureGateResult::Allowed);
}
...
fn capability_mapping_serde_round_trip() {
    ...
    assert_eq!(back.required_entitlement, Some(Entitlement::GovernanceCompliance));
...
fn entitlements_are_independently_isolated() {
    ...
    assert_eq!(gate.check(&gov_only, "advanced_audit_export"), FeatureGateResult::Allowed);
    assert!(matches!(gate.check(&gov_only, "advanced_admin"), FeatureGateResult::Denied { .. }),
    ...
    assert_eq!(gate.check(&adm_only, "advanced_admin"), FeatureGateResult::Allowed);
    assert!(matches!(gate.check(&adm_only, "advanced_audit_export"), FeatureGateResult::Denied { .. }),
