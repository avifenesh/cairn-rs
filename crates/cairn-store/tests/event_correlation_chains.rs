...
use cairn_domain::{
    CommandId, EventEnvelope, EventId, EventSource, ProjectId, ProjectKey, RunCreated,
    RunId, RuntimeEvent, SessionCreated, SessionId, TaskCreated, TaskId, TenantId,
    WorkspaceId,
};
use cairn_store::{
    EntityRef, EventLog, InMemoryStore,
};
...
async fn correlation_chain_links_session_run_task() {
    ...
    assert!(chain1.iter().any(|e| matches!(&e.envelope.payload, RuntimeEvent::SessionCreated(_))));
    assert!(chain1.iter().any(|e| matches!(&e.envelope.payload, RuntimeEvent::RunCreated(_))));
    assert!(chain1.iter().any(|e| matches!(&e.envelope.payload, RuntimeEvent::TaskCreated(_))));
}
...
async fn different_correlation_ids_are_isolated() {
    ...
    let chain1: Vec<_> = all.iter()
        .filter(|e| e.envelope.correlation_id.as_deref() == Some("chain_1"))
        .collect();
    let chain2: Vec<_> = all.iter()
        .filter(|e| e.envelope.correlation_id.as_deref() == Some("chain_2"))
        .collect();
    let uncorrelated: Vec<_> = all.iter()
    ...
    assert!(chain1.iter().all(|e| e.envelope.correlation_id.as_deref() == Some("chain_1")));
    assert!(chain2.iter().all(|e| e.envelope.correlation_id.as_deref() == Some("chain_2")));
}
...
async fn causation_id_links_cause_to_effect() {
    ...
    let pos = store.find_by_causation_id("cmd_root_001")
        .await.unwrap()
    ...
    let all = store.read_stream(None, 100).await.unwrap();
    let evt_a_stored = all.iter().find(|e| e.envelope.event_id.as_str() == "evt_cause_a").unwrap();
    assert!(evt_a_stored.envelope.causation_id.is_none(),
        "root event has no causation_id");

    // Event B carries the causation_id.
    let evt_b_stored = all.iter().find(|e| e.envelope.event_id.as_str() == "evt_effect_b").unwrap();
    assert_eq!(
        evt_b_stored.envelope.causation_id.as_ref().map(|c| c.as_str()),
        Some("cmd_root_001"),
...
async fn multi_hop_causation_chain() {
    ...
    store.append(&[
        session_evt("e_hop_1", "sess_hop")
            .with_causation_id(CommandId::new("cmd_hop_001")),
    ...
    let pos1 = store.find_by_causation_id("cmd_hop_001").await.unwrap().unwrap();
    let pos2 = store.find_by_causation_id("cmd_hop_002").await.unwrap().unwrap();
    let pos3 = store.find_by_causation_id("cmd_hop_003").await.unwrap().unwrap();
...
async fn find_by_causation_id_returns_first_match() {
    ...
    let pos_a = store.find_by_causation_id("cmd_fbc_a").await.unwrap().unwrap();
    assert_eq!(pos_a.0, 1);

    // cmd_fbc_b → position 2 (RunCreated).
    let pos_b = store.find_by_causation_id("cmd_fbc_b").await.unwrap().unwrap();
    assert_eq!(pos_b.0, 2);

    assert_ne!(pos_a, pos_b, "different causation_ids map to different positions");
}
...
async fn correlation_id_persists_through_read_by_entity() {
    ...
    store.append(&[
        session_evt("e_s", "sess_entity_corr")
    ...
    let sess_events = store.read_by_entity(
        &EntityRef::Session(sess_id.clone()),
        None,
        10,
    ).await.unwrap();
    ...
    let run_events = store.read_by_entity(
        &EntityRef::Run(RunId::new("run_entity_corr")),
        None,
        10,
    ).await.unwrap();
...
async fn correlation_chain_visible_in_partial_replay() {
    ...
    let partial = store.read_stream(Some(EventPosition(1)), 100).await.unwrap();
    assert_eq!(partial.len(), 4, "events at positions 2-5");

    let chain: Vec<_> = partial.iter()
        .filter(|e| e.envelope.correlation_id.as_deref() == Some("replay_chain"))
        .collect();
    assert_eq!(chain.len(), 3, "replay from pos 1 includes all 3 chain events");
}
...
async fn uncorrelated_event_has_none_not_empty_string() {
    ...
    store.append(&[session_evt("e1", "sess_no_corr")]).await.unwrap();

    let events = store.read_stream(None, 10).await.unwrap();
    assert_eq!(events.len(), 1);
    assert!(events[0].envelope.correlation_id.is_none(),
        "no correlation → None, not Some(\"\")");
    assert!(events[0].envelope.causation_id.is_none(),
        "no causation → None");
}
