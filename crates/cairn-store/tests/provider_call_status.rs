...
use cairn_domain::{
    ...
    RouteDecisionId, RuntimeEvent, TenantId, WorkspaceId,
};
use cairn_store::{projections::ProviderCallReadModel, EventLog, InMemoryStore};
...
async fn succeeded_call_stores_all_fields() {
    ...
    assert!(
        ...
        "succeeded call has no error class"
    );
}
...
async fn list_by_project_returns_only_project_calls() {
    ...
    assert!(
        !ids_a.contains(&"call_b1"),
        "proj_b call must not appear in proj_a"
    );
...
async fn cost_aggregation_across_multiple_calls() {
    ...
    for (i, &cost) in costs.iter().enumerate() {
        let r =
            ProviderCallReadModel::get(&store, &ProviderCallId::new(format!("call_cost_{i:02}")))
                .await
                .unwrap()
                .unwrap();
        assert_eq!(
            r.cost_micros,
            Some(cost),
            "call {i}: cost_micros must be {cost}"
        );
    }
    ...
    let actual_total: u64 = all_calls.iter().filter_map(|c| c.cost_micros).sum();
    assert_eq!(
...
async fn rate_limited_call_stores_correctly() {
    ...
    store
        .append(&[call(
            ...
            None,
            None,
            None, // rate-limit response is fast
    ...
    assert_eq!(
        r.latency_ms,
        Some(12),
        "rate-limited calls still have latency"
    );
}
