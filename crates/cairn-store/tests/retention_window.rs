...
use cairn_domain::{
    EventEnvelope, EventId, EventSource, ProjectKey, RetentionPolicySet, RunCreated,
    RunId, RuntimeEvent, SessionCreated, SessionId, TenantId,
};
use cairn_store::{
...
fn tenant() -> TenantId { TenantId::new("tenant_ret") }
...
async fn append_50_events(store: &Arc<InMemoryStore>) -> EventPosition {
    for i in 0..10u32 {
        ...
        let mut batch = vec![
            ev(&format!("sess_{i}"), RuntimeEvent::SessionCreated(SessionCreated {
                project: project(i),
        ...
        for j in 0..2u32 {
            let n = i * 2 + j;
            batch.push(ev(&format!("run_{n}"), RuntimeEvent::RunCreated(RunCreated {
                project: project(i),
                session_id: sess.clone(),
    ...
    EventLog::head_position(store.as_ref()).await.unwrap().unwrap()
}
...
async fn fifty_events_read_stream_correct_count() {
    ...
    assert_eq!(all.len(), 50, "read_stream(limit=100) must return all 50 events");

    // Exactly 10.
    let ten = EventLog::read_stream(store.as_ref(), None, 10).await.unwrap();
    assert_eq!(ten.len(), 10, "read_stream(limit=10) must return exactly 10 events");

    // Exactly 1.
    let one = EventLog::read_stream(store.as_ref(), None, 1).await.unwrap();
    assert_eq!(one.len(), 1);

    // After position 40 → 10 remaining.
    let tail = EventLog::read_stream(store.as_ref(), Some(EventPosition(40)), 100).await.unwrap();
    assert_eq!(tail.len(), 10, "10 events remain after position 40");

    // After last position → empty.
    let empty = EventLog::read_stream(store.as_ref(), Some(EventPosition(50)), 100).await.unwrap();
...
async fn head_position_equals_fifty_after_fifty_appends() {
    ...
    assert_eq!(head, EventPosition(50), "head_position must be 50 after 50 appends");

    // head_position always reflects the latest append.
    store.append(&[ev("extra", RuntimeEvent::SessionCreated(SessionCreated {
        project: project(99),
        session_id: SessionId::new("sess_extra"),
    }))]).await.unwrap();
    let head51 = EventLog::head_position(store.as_ref()).await.unwrap().unwrap();
    assert_eq!(head51, EventPosition(51), "head_position advances to 51 after one more append");
}
...
async fn events_older_than_cutoff_are_identifiable() {
    ...
    let older_than_future: Vec<_> = events.iter()
    ...
    let at_earliest: Vec<_> = events.iter()
        .filter(|e| e.stored_at >= min_stored_at)
        .collect();
    assert_eq!(at_earliest.len(), 50, "all events are at or after the earliest stored_at");
    ...
    let within_window: Vec<_> = events.iter()
        .filter(|e| e.stored_at >= replay_cutoff)
        .collect();
    assert_eq!(
        within_window.len(), 50,
...
async fn retention_doesnt_break_entity_scoped_reads() {
    ...
    for i in 0..5u32 {
        store.append(&[ev(&format!("sess_prune_{i}"), RuntimeEvent::SessionCreated(SessionCreated {
            project: project(0),
            session_id: sess.clone(),
        }))]).await.unwrap();
    }
    ...
    store.append(&[ev("unrelated", RuntimeEvent::SessionCreated(SessionCreated {
        project: project(1),
        session_id: SessionId::new("sess_unrelated"),
    }))]).await.unwrap();

    // Set a retention policy: keep at most 2 events per entity.
    store.append(&[ev("policy_set", RuntimeEvent::RetentionPolicySet(RetentionPolicySet {
        tenant_id: tenant(),
    ...
    let before = EventLog::read_stream(store.as_ref(), None, 100).await.unwrap().len();
    assert_eq!(before, 7, "7 events before retention: 5 duplicates + 1 unrelated + 1 policy");

    // Apply retention for the tenant.
    let result = RetentionMaintenance::apply_retention(store.as_ref(), &tenant()).await.unwrap();
    ...
    assert!(result.events_pruned >= 3, "at least 3 events must be pruned (5 - 2 = 3)");

    // After retention: total event count must drop.
    let after = EventLog::read_stream(store.as_ref(), None, 100).await.unwrap();
    assert!(
        after.len() < before,
        "event log must shrink after apply_retention: {before} → {}", after.len()
    );
    ...
    let prune_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Session(sess),
        None, 100,
    ).await.unwrap();
    assert!(
...
async fn seventy_two_hour_sse_replay_window_contract() {
    ...
    for reconnect_at in [0u64, 1, 10, 25, 40, 49] {
        ...
        if !replay.is_empty() {
            assert_eq!(
                replay[0].position.0, reconnect_at + 1,
