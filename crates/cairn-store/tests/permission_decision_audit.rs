...
fn project_a() -> ProjectKey { ProjectKey::new("tenant_perm", "ws_perm", "proj_a") }
fn project_b() -> ProjectKey { ProjectKey::new("tenant_perm", "ws_perm", "proj_b") }
...
async fn all_permission_events(store: &Arc<InMemoryStore>)
    -> Vec<cairn_store::event_log::StoredEvent>
{
    EventLog::read_stream(store.as_ref(), None, 200)
        .await.unwrap()
        .into_iter()
        .filter(|e| matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(_)))
        .collect()
}
...
async fn permission_allowed_persists_in_log() {
    ...
    store.append(&[allow_event(
        "dec_001", "agent:run_1", "write", "file:/tmp/output.json", None, 5_000
    )]).await.unwrap();
...
async fn permission_denied_is_stored_and_distinguishable() {
    ...
    store.append(&[
        ...
        allow_event("dec_c", "agent:run_2", "read",  "file:/config.json", None, 3_000),
    ]).await.unwrap();
...
async fn read_by_entity_scoping_for_tool_invocation() {
    ...
    store.append(&[
        ev("sess_scope", RuntimeEvent::SessionCreated(SessionCreated {
            project: project_a(), session_id: SessionId::new("sess_perm_scope"),
        })),
        ev("run_scope", RuntimeEvent::RunCreated(RunCreated {
            project: project_a(), session_id: SessionId::new("sess_perm_scope"),
            run_id: RunId::new("run_perm_scope"), parent_run_id: None,
    ...
    store.append(&[allow_event(
        "dec_scoped", "agent:run_perm_scope",
        "execute", "tool:file_write",
        Some(inv_id), 10_000,
    )]).await.unwrap();
    ...
    assert_eq!(inv_events.len(), 1, "one ToolInvocationStarted event for this invocation");
    assert!(
    ...
    let perm_for_inv: Vec<_> = all_permission_events(&store).await
        .into_iter()
        .filter(|e| {
            matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
        ...
        .collect();
    assert_eq!(perm_for_inv.len(), 1, "one permission decision linked to this invocation");
}
...
async fn invocation_id_links_permission_to_tool_call() {
    ...
    store.append(&[
        ...
        allow_event("dec_tool_1", "agent:x", "read",  "db://records", Some("inv_tool_1"), 1_000),
        deny_event( "dec_tool_2", "agent:x", "write", "db://records", Some("inv_tool_2"), 2_000),
    ]).await.unwrap();
    ...
    let perm_1: Vec<_> = all_perms.iter().filter(|e| {
        matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
    ...
    let perm_2: Vec<_> = all_perms.iter().filter(|e| {
        matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
    ...
    store.append(&[allow_event("dec_no_inv", "agent:y", "read", "file:/log", None, 3_000)])
        .await.unwrap();
    let no_inv: Vec<_> = all_permission_events(&store).await
        .into_iter()
        .filter(|e| {
            matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
        ...
        .collect();
    assert_eq!(no_inv.len(), 1, "one permission decision without invocation_id");
}
...
async fn cross_project_isolation_via_principal_filtering() {
    ...
    store.append(&[
        allow_event("dec_a1", "proj_a:run_1", "read",  "file:/a.json", None, 1_000),
        allow_event("dec_a2", "proj_a:run_1", "write", "file:/a.json", None, 2_000),
        deny_event( "dec_a3", "proj_a:run_1", "exec",  "tool:shell",  None, 3_000),
    ]).await.unwrap();

    // Project B decisions (principal contains "proj_b").
    store.append(&[
        allow_event("dec_b1", "proj_b:run_2", "read",  "file:/b.json", None, 4_000),
        deny_event( "dec_b2", "proj_b:run_2", "write", "db://secret",  None, 5_000),
    ]).await.unwrap();
    ...
    let proj_a_perms: Vec<_> = all_perms.iter().filter(|e| {
        matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
    ...
    let proj_b_perms: Vec<_> = all_perms.iter().filter(|e| {
        matches!(&e.envelope.payload, RuntimeEvent::PermissionDecisionRecorded(d)
