...
use cairn_domain::{
    EvalDatasetCreated, EvalDatasetEntryAdded, EventEnvelope, EventId, EventSource, RuntimeEvent,
    TenantId,
};
use cairn_store::{projections::EvalDatasetReadModel, EventLog, InMemoryStore};
...
async fn eval_dataset_created_stores_record() {
    ...
    store
        .append(&[create_dataset("e1", "ds_001", "QA Benchmark", ts)])
        .await
        .unwrap();
...
async fn dataset_name_persists_correctly() {
    ...
    store
        .append(&[create_dataset("e1", "ds_name", long_name, ts)])
        .await
        .unwrap();
...
async fn eval_dataset_entry_added_increments_count() {
    ...
    store
        .append(&[create_dataset("e0", "ds_entries", "Entry Test", ts)])
        .await
        .unwrap();
    ...
    // Add first entry.
    store
        .append(&[add_entry("e1", "ds_entries", "entry_001", ts + 1)])
        .await
        .unwrap();
    ...
    let after_three = EvalDatasetReadModel::get_dataset(&store, "ds_entries")
        ...
        .unwrap();
    assert_eq!(
        after_three.entries.len(),
        3,
        "three entries after three appends"
    );
}
...
async fn entry_count_reflects_all_appended_entries() {
    ...
    store
        .append(&[create_dataset("e0", "ds_count", "Count Test", ts)])
        .await
        .unwrap();
...
async fn duplicate_entry_ids_are_deduplicated() {
    ...
    store
        .append(&[create_dataset("e0", "ds_dedup", "Dedup Test", ts)])
        .await
        .unwrap();
...
async fn multiple_datasets_tracked_independently() {
    ...
    let ds_a = EvalDatasetReadModel::get_dataset(&store, "ds_a")
        .await
        .unwrap()
        .unwrap();
    let ds_b = EvalDatasetReadModel::get_dataset(&store, "ds_b")
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        ds_b.entries.len(),
        0,
        "ds_b has no entries — they stay independent"
    );
}
...
async fn entries_are_scoped_to_their_dataset() {
    ...
    let s1 = EvalDatasetReadModel::get_dataset(&store, "ds_scope_1")
        .await
        .unwrap()
        .unwrap();
    let s2 = EvalDatasetReadModel::get_dataset(&store, "ds_scope_2")
        .await
        .unwrap()
        .unwrap();
    ...
    assert!(!s1
        .entries
        .iter()
        .any(|e| e.tags.contains(&"entry_s2".to_owned())));
    assert!(!s1
        .entries
        .iter()
        .any(|e| e.tags.contains(&"entry_s3".to_owned())));
}
...
async fn list_by_tenant_returns_datasets_sorted_by_created_at() {
    ...
    let datasets = EvalDatasetReadModel::list_by_tenant(&store, &TenantId::new(""), 10, 0)
        .await
        .unwrap();
