...
//!   3. Appends the same event a second time and asserts the table
//!      stays at one row, proving `ON CONFLICT DO NOTHING` works.
//!
//! Only the SQLite backend is exercised here because the workspace
//! does not stand up a live Postgres instance in tests. Parity with
//! Postgres is enforced by:
//!
//!   * `schema_parity.rs` — both backends define the same tables.
//!   * `pg_migration_contract.rs` — V026 is registered and non-empty.
//!   * the pg migration SQL itself mirrors the SQLite DDL with the
//!     only intentional differences being type names (BOOLEAN / BIGINT
//!     vs INTEGER) and parameter style ($1 vs ?).
...
async fn recovery_completed_is_projected_with_recovered_flag() {
    ...
    let rows: Vec<(String, Option<String>, Option<String>, i64, Option<String>)> = sqlx::query_as(
...
fn sample_decision(id: &str, outcome: DecisionOutcome, cached: bool) -> EventEnvelope<RuntimeEvent> {
...
async fn decision_recorded_is_projected_with_outcome_kind() {
    ...
    .await
    .unwrap();
    let parsed: DecisionKey = serde_json::from_str(&dk_json).expect("decision_key_json is valid");
