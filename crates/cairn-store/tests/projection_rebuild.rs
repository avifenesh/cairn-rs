...
fn task_transition(
    ...
    evt(
        evt_id,
        RuntimeEvent::TaskStateChanged(cairn_domain::TaskStateChanged {
            project: project_a(),
...
fn build_20_events() -> Vec<EventEnvelope<RuntimeEvent>> {
    vec![
        ...
        evt(
            ...
            RuntimeEvent::SessionCreated(SessionCreated {
                project: project_b(),
                session_id: SessionId::new("sess_b1"),
            }),
        ),
        ...
        evt(
            ...
            RuntimeEvent::ApprovalRequested(ApprovalRequested {
                project: project_a(),
                approval_id: ApprovalId::new("appr_2"),
...
async fn snapshot_runs(store: &InMemoryStore) -> Vec<RunSnap> {
    ...
    for run_id in ["run_1", "run_2", "run_3", "run_4_child"] {
        if let Some(r) = RunReadModel::get(store, &RunId::new(run_id)).await.unwrap() {
            snaps.push(RunSnap {
...
async fn projection_rebuild_parity_after_replay() {
    ...
    assert_eq!(
        sessions1, sessions2,
        "SESSION projections must be identical after replay"
    );
    assert_eq!(
        runs1, runs2,
        "RUN projections must be identical after replay"
    );
    assert_eq!(
        tasks1, tasks2,
        "TASK projections must be identical after replay"
    );
    assert_eq!(
        approvals1, approvals2,
        "APPROVAL projections must be identical after replay"
    );
...
async fn event_log_order_is_preserved_on_replay() {
    ...
    for (i, (e1, e2)) in log1.iter().zip(log2.iter()).enumerate() {
        assert_eq!(e1.position, e2.position, "event {i}: positions must match");
        assert_eq!(
            e1.envelope.event_id, e2.envelope.event_id,
            "event {i}: event_ids must match"
        );
        // Payload equality (excludes stored_at which is timing-dependent).
        assert_eq!(
            e1.envelope.payload, e2.envelope.payload,
...
async fn run_state_correct_after_replay() {
    ...
    let r1 = RunReadModel::get(&store, &RunId::new("run_1"))
        .await
        .unwrap()
        .unwrap();
    ...
    let r2 = RunReadModel::get(&store, &RunId::new("run_2"))
        .await
        .unwrap()
        .unwrap();
    ...
    let r3 = RunReadModel::get(&store, &RunId::new("run_3"))
        .await
        .unwrap()
        .unwrap();
    ...
    let r4 = RunReadModel::get(&store, &RunId::new("run_4_child"))
        .await
        .unwrap()
        .unwrap();
...
async fn task_state_correct_after_replay() {
    ...
    let t1 = TaskReadModel::get(&store, &TaskId::new("task_1"))
        .await
        .unwrap()
        .unwrap();
    ...
    let t2 = TaskReadModel::get(&store, &TaskId::new("task_2"))
        .await
        .unwrap()
        .unwrap();
    ...
    let t3 = TaskReadModel::get(&store, &TaskId::new("task_3"))
        .await
        .unwrap()
        .unwrap();
...
async fn approval_state_correct_after_replay() {
    ...
    let a1 = ApprovalReadModel::get(&store, &ApprovalId::new("appr_1"))
        .await
        .unwrap()
        .unwrap();
    ...
    let a2 = ApprovalReadModel::get(&store, &ApprovalId::new("appr_2"))
        .await
        .unwrap()
        .unwrap();
    ...
    assert!(
        pending.is_empty(),
        "both approvals resolved — pending must be empty"
    );
}
...
async fn cross_project_isolation_preserved_on_replay() {
    ...
    let proj_a_sessions = SessionReadModel::list_by_project(&store, &project_a(), 10, 0)
        .await
        .unwrap();
    assert_eq!(proj_a_sessions.len(), 2);
    let ids: Vec<_> = proj_a_sessions
        .iter()
    ...
    let proj_b_sessions = SessionReadModel::list_by_project(&store, &project_b(), 10, 0)
        .await
        .unwrap();
