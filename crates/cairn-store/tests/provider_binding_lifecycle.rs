...
fn register_connection(
    ...
    evt(
        ...
        RuntimeEvent::ProviderConnectionRegistered(ProviderConnectionRegistered {
            tenant: TenantKey { tenant_id: TenantId::new(tenant) },
...
async fn provider_connection_registered_is_stored() {
    ...
    store
        .append(&[register_connection("e1", "conn_openai", "t_bind", "openai", ts)])
        .await
        .unwrap();

    let record = ProviderConnectionReadModel::get(&store, &conn_id)
        .await.unwrap().expect("connection must exist after registration");
...
async fn provider_binding_created_links_connection_and_model() {
    ...
    store
        .append(&[
            ...
            create_binding("e2", "bind_001", "conn_a1", "gpt-4o", OperationKind::Generate, true, ts + 1),
        ])
        .await
        .unwrap();

    let record = ProviderBindingReadModel::get(&store, &binding_id)
        .await.unwrap().expect("binding must exist after creation");
...
async fn binding_carries_all_settings_fields() {
    ...
    store
        .append(&[evt("e1", RuntimeEvent::ProviderBindingCreated(ProviderBindingCreated {
            project: default_project(),
            provider_binding_id:    ProviderBindingId::new("bind_full"),
            provider_connection_id: ProviderConnectionId::new("conn_full"),
            provider_model_id:      ProviderModelId::new("claude-3-5-sonnet-20241022"),
    ...
    assert_eq!(r.settings.structured_output_mode, StructuredOutputMode::Preferred);
...
async fn binding_state_changed_to_inactive() {
    ...
    store
        .append(&[
            ...
            create_binding("e2", "bind_deact", "conn_deact", "claude-3-haiku-20240307",
                OperationKind::Generate, true, ts + 1),
        ])
        .await
        .unwrap();
    ...
    let after = ProviderBindingReadModel::get(&store, &ProviderBindingId::new("bind_deact"))
        .await.unwrap().unwrap();
    assert!(!after.active, "binding must be inactive after ProviderBindingStateChanged");
}
...
async fn binding_state_can_be_toggled_active_inactive_active() {
    ...
    store
        .append(&[
            create_binding("e1", "bind_toggle", "conn_tog", "gpt-4o-mini",
                OperationKind::Generate, true, ts),
        ])
        .await
        .unwrap();

    // active → inactive.
    store.append(&[change_binding_state("e2", "bind_toggle", false)]).await.unwrap();
    let r = ProviderBindingReadModel::get(&store, &ProviderBindingId::new("bind_toggle"))
        .await.unwrap().unwrap();
    assert!(!r.active);

    // inactive → active.
    store.append(&[change_binding_state("e3", "bind_toggle", true)]).await.unwrap();
    let r = ProviderBindingReadModel::get(&store, &ProviderBindingId::new("bind_toggle"))
        .await.unwrap().unwrap();
...
async fn list_by_project_returns_only_project_bindings() {
    ...
    store
        .append(&[
            evt("e1", RuntimeEvent::ProviderBindingCreated(ProviderBindingCreated {
            ...
            evt("e2", RuntimeEvent::ProviderBindingCreated(ProviderBindingCreated {
                project: proj_a.clone(),
            ...
            evt("e3", RuntimeEvent::ProviderBindingCreated(ProviderBindingCreated {
                ...
                provider_model_id:      ProviderModelId::new("claude-3-haiku-20240307"),
                operation_kind:         OperationKind::Generate,
                settings:               default_settings(),
                policy_id:              None,
                active:                 true,
    ...
    let ids_a: std::collections::HashSet<_> =
        bindings_a.iter().map(|b| b.provider_binding_id.as_str()).collect();
    assert!(ids_a.contains(&"bind_a1"));
    assert!(ids_a.contains(&"bind_a2"));
    assert!(!ids_a.contains(&"bind_b1"), "project B binding must not appear in A");
...
async fn list_active_returns_only_active_bindings_for_operation() {
    ...
    store
        .append(&[
            create_binding("e1", "gen_a", "conn_1", "gpt-4o",        OperationKind::Generate, true,  ts),
            create_binding("e2", "gen_b", "conn_2", "gpt-4o-mini",   OperationKind::Generate, true,  ts + 1),
            create_binding("e3", "gen_c", "conn_3", "claude-haiku",   OperationKind::Generate, false, ts + 2),
            create_binding("e4", "emb_a", "conn_1", "text-embedding", OperationKind::Embed,    true,  ts + 3),
        ])
        .await
        .unwrap();

    // list_active(Generate) returns only active generate bindings.
    let active_gen = ProviderBindingReadModel::list_active(
        &store, &default_project(), OperationKind::Generate,
    ).await.unwrap();
    assert_eq!(active_gen.len(), 2, "2 active generate bindings");
    let gen_ids: Vec<_> = active_gen.iter().map(|b| b.provider_binding_id.as_str()).collect();
    assert!(gen_ids.contains(&"gen_a"));
    assert!(gen_ids.contains(&"gen_b"));
    assert!(!gen_ids.contains(&"gen_c"), "inactive binding excluded from routing candidates");
    assert!(!gen_ids.contains(&"emb_a"), "embed binding excluded from generate results");

    // list_active(Embed) returns only active embed binding.
    let active_emb = ProviderBindingReadModel::list_active(
        &store, &default_project(), OperationKind::Embed,
    ).await.unwrap();
    ...
    let all = ProviderBindingReadModel::list_by_project(&store, &default_project(), 10, 0)
        .await.unwrap();
    assert_eq!(all.len(), 4, "list_by_project includes inactive bindings for audit");
}
...
async fn deactivating_binding_removes_it_from_routing_candidates() {
    ...
    store
        .append(&[
            create_binding("e1", "bind_pri_1", "conn_p1", "gpt-4o",      OperationKind::Generate, true, ts),
            create_binding("e2", "bind_pri_2", "conn_p2", "gpt-4o-mini", OperationKind::Generate, true, ts + 1),
            create_binding("e3", "bind_pri_3", "conn_p3", "claude-haiku", OperationKind::Generate, true, ts + 2),
        ])
        .await
        .unwrap();

    // Initially all 3 active.
    let before = ProviderBindingReadModel::list_active(
        &store, &default_project(), OperationKind::Generate,
    ).await.unwrap();
    assert_eq!(before.len(), 3, "all 3 bindings are routing candidates");

    // Deactivate the primary binding (simulate failover).
    store.append(&[change_binding_state("e4", "bind_pri_1", false)]).await.unwrap();

    let after = ProviderBindingReadModel::list_active(
        &store, &default_project(), OperationKind::Generate,
    ).await.unwrap();
    assert_eq!(after.len(), 2, "deactivated binding removed from routing candidates");
    let remaining: Vec<_> = after.iter().map(|b| b.provider_binding_id.as_str()).collect();
    assert!(!remaining.contains(&"bind_pri_1"), "deactivated binding excluded");
...
async fn connections_listed_by_tenant() {
    ...
    let t1_conns = ProviderConnectionReadModel::list_by_tenant(
        &store, &TenantId::new("tenant_one"), 10, 0,
    ).await.unwrap();
    assert_eq!(t1_conns.len(), 2);
    assert!(t1_conns.iter().all(|c| c.tenant_id.as_str() == "tenant_one"));

    let t2_conns = ProviderConnectionReadModel::list_by_tenant(
        &store, &TenantId::new("tenant_two"), 10, 0,
    ).await.unwrap();
...
async fn list_active_is_scoped_by_operation_kind() {
    ...
    for (i, op) in [(0, OperationKind::Generate), (1, OperationKind::Embed), (2, OperationKind::Rerank)] {
        store
            .append(&[evt(
                &format!("e{i}"),
                RuntimeEvent::ProviderBindingCreated(ProviderBindingCreated {
    ...
    for (i, op) in [(0, OperationKind::Generate), (1, OperationKind::Embed), (2, OperationKind::Rerank)] {
        let results = ProviderBindingReadModel::list_active(
            &store, &default_project(), op,
        ).await.unwrap();
        assert_eq!(results.len(), 1, "{op:?} must return exactly 1 binding");
        assert_eq!(results[0].operation_kind, op);
        assert_eq!(results[0].provider_binding_id.as_str(), format!("bind_op_{i}"));
    }
}
