...
fn ingest(
    ...
    evt(evt_id, RuntimeEvent::SignalIngested(SignalIngested {
        project:      proj,
...
async fn list_by_project_returns_only_matching_project_signals() {
    ...
    store.append(&[
        ingest("e1", "sig_a1", proj_a.clone(), "src_a", serde_json::json!({"n": 1}), ts),
        ingest("e2", "sig_a2", proj_a.clone(), "src_a", serde_json::json!({"n": 2}), ts + 1),
        ingest("e3", "sig_b1", proj_b.clone(), "src_b", serde_json::json!({"n": 3}), ts + 2),
    ]).await.unwrap();
    ...
    assert!(!a_ids.contains(&"sig_b1"), "proj_b signal must not appear in proj_a");
    ...
    let empty = SignalReadModel::list_by_project(
        &store, &project("tx", "wx", "px"), 10, 0,
    ).await.unwrap();
...
async fn payload_round_trips_primitive_types() {
    ...
    store.append(&[ingest(
        "e1", "sig_prim", default_project(), "test", payload.clone(), ts,
    )]).await.unwrap();
...
async fn payload_round_trips_nested_objects() {
    ...
    store.append(&[ingest(
        "e1", "sig_nested", default_project(), "github", payload.clone(), ts,
    )]).await.unwrap();

    let record = SignalReadModel::get(&store, &SignalId::new("sig_nested"))
        .await.unwrap().unwrap();

    assert_eq!(record.payload, payload, "nested payload must round-trip exactly");
...
async fn empty_payload_object_is_valid() {
    ...
    store.append(&[ingest(
        "e1", "sig_empty", default_project(), "scheduler", serde_json::json!({}), ts,
    )]).await.unwrap();

    let record = SignalReadModel::get(&store, &SignalId::new("sig_empty"))
        .await.unwrap().unwrap();
    assert!(record.payload.is_object());
    assert_eq!(record.payload.as_object().unwrap().len(), 0,
...
async fn array_payload_is_valid() {
    ...
    store.append(&[ingest(
        "e1", "sig_arr", default_project(), "batch", payload.clone(), ts,
    )]).await.unwrap();
...
async fn list_by_project_sorted_by_timestamp_ms_ascending() {
    ...
    store.append(&[
        ingest("e1", "sig_ts3", default_project(), "s", serde_json::json!({"i":3}), ts + 200),
        ingest("e2", "sig_ts1", default_project(), "s", serde_json::json!({"i":1}), ts),
        ingest("e3", "sig_ts2", default_project(), "s", serde_json::json!({"i":2}), ts + 100),
    ]).await.unwrap();
    ...
    assert_eq!(signals[0].id.as_str(), "sig_ts1", "earliest timestamp first");
...
async fn signals_with_same_timestamp_all_returned() {
    ...
    store.append(&[
        ingest("e1", "sig_same_1", default_project(), "batch", serde_json::json!({"seq":1}), ts),
        ingest("e2", "sig_same_2", default_project(), "batch", serde_json::json!({"seq":2}), ts),
        ingest("e3", "sig_same_3", default_project(), "batch", serde_json::json!({"seq":3}), ts),
    ]).await.unwrap();

    let signals = SignalReadModel::list_by_project(&store, &default_project(), 10, 0)
        .await.unwrap();
    assert_eq!(signals.len(), 3, "all 3 signals with same timestamp are returned");
}
...
async fn source_field_accepts_various_formats() {
    ...
    for (i, (sig_id, source)) in sources.iter().enumerate() {
        store.append(&[ingest(
            &format!("e{i}"), sig_id, default_project(),
            source, serde_json::json!({}), ts + i as u64,
        )]).await.unwrap();
    }
