...
use cairn_domain::{
    ...
    RunId, RuntimeEvent, SessionCreated, SessionId, StateTransition, TaskCreated, TaskId,
    TaskState, TaskStateChanged, TenantId, WorkspaceId,
};
use cairn_domain::lifecycle::can_transition_task_state;
use cairn_store::{
    projections::TaskReadModel,
...
fn task_transition(
    ...
    evt(evt_id, RuntimeEvent::TaskStateChanged(TaskStateChanged {
        project: project(),
...
fn setup(run_id: &str, task_id: &str) -> Vec<EventEnvelope<RuntimeEvent>> {
    ...
    vec![
        evt("e_s", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: SessionId::new(&sess),
        })),
        evt("e_r", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new(&sess),
            run_id: RunId::new(run_id), parent_run_id: None,
            prompt_release_id: None, agent_role_id: None,
        })),
        evt("e_t", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(),
            task_id: TaskId::new(task_id),
...
async fn task_created_has_queued_state() {
    ...
    let record = TaskReadModel::get(&store, &TaskId::new("task_q"))
        .await.unwrap().expect("TaskRecord must exist after TaskCreated");
...
async fn task_transitions_queued_to_leased_to_running() {
    ...
    store.append(&[
        task_transition("t1", "task_qlr", Some(TaskState::Queued), TaskState::Leased, None),
    ]).await.unwrap();
    let leased = TaskReadModel::get(&store, &TaskId::new("task_qlr")).await.unwrap().unwrap();
    assert_eq!(leased.state, TaskState::Leased);
    assert_eq!(leased.version, 2);
    assert!(!leased.state.is_terminal());

    store.append(&[
        task_transition("t2", "task_qlr", Some(TaskState::Leased), TaskState::Running, None),
    ]).await.unwrap();
    let running = TaskReadModel::get(&store, &TaskId::new("task_qlr")).await.unwrap().unwrap();
...
async fn task_transitions_running_to_waiting_approval() {
    ...
    store.append(&[
        task_transition("t1", "task_wa", Some(TaskState::Queued),  TaskState::Leased,  None),
        task_transition("t2", "task_wa", Some(TaskState::Leased),  TaskState::Running, None),
        task_transition("t3", "task_wa", Some(TaskState::Running), TaskState::WaitingApproval, None),
    ]).await.unwrap();

    let record = TaskReadModel::get(&store, &TaskId::new("task_wa")).await.unwrap().unwrap();
    assert_eq!(record.state, TaskState::WaitingApproval,
...
async fn task_completes_after_approval() {
    ...
    store.append(&[
        task_transition("t1", "task_done", Some(TaskState::Queued),          TaskState::Leased,           None),
        task_transition("t2", "task_done", Some(TaskState::Leased),          TaskState::Running,          None),
        task_transition("t3", "task_done", Some(TaskState::Running),         TaskState::WaitingApproval,  None),
        task_transition("t4", "task_done", Some(TaskState::WaitingApproval), TaskState::Running,          None),
        task_transition("t5", "task_done", Some(TaskState::Running),         TaskState::Completed,        None),
    ]).await.unwrap();

    let record = TaskReadModel::get(&store, &TaskId::new("task_done")).await.unwrap().unwrap();
...
async fn task_fails_with_execution_error() {
    ...
    store.append(&[
        task_transition("t1", "task_fail", Some(TaskState::Queued),  TaskState::Leased,  None),
        task_transition("t2", "task_fail", Some(TaskState::Leased),  TaskState::Running, None),
        evt("t3", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(),
    ...
    let record = TaskReadModel::get(&store, &TaskId::new("task_fail")).await.unwrap().unwrap();
...
async fn task_fails_when_approval_rejected() {
    ...
    store.append(&[
        task_transition("t1", "task_rej", Some(TaskState::Queued),          TaskState::Leased,          None),
        task_transition("t2", "task_rej", Some(TaskState::Leased),          TaskState::Running,         None),
        task_transition("t3", "task_rej", Some(TaskState::Running),         TaskState::WaitingApproval, None),
        evt("t4", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(),
    ...
    let record = TaskReadModel::get(&store, &TaskId::new("task_rej")).await.unwrap().unwrap();
    assert_eq!(record.state, TaskState::Failed);
...
async fn retryable_failure_increments_retry_count() {
    let store = InMemoryStore::new();
    store.append(&setup("run_retry", "task_retry")).await.unwrap();

    // First attempt: fails with retryable error.
    store.append(&[
        task_transition("t1", "task_retry", Some(TaskState::Queued),  TaskState::Leased,          None),
        task_transition("t2", "task_retry", Some(TaskState::Leased),  TaskState::Running,         None),
        task_transition("t3", "task_retry", Some(TaskState::Running), TaskState::RetryableFailed, None),
    ]).await.unwrap();
    ...
    assert_eq!(after_first_fail.retry_count, 1, "first retryable failure = retry_count 1");
    assert!(!after_first_fail.state.is_terminal(), "RetryableFailed is non-terminal");

    // Re-queue and retry.
    store.append(&[
        task_transition("t4", "task_retry", Some(TaskState::RetryableFailed), TaskState::Queued,  None),
        task_transition("t5", "task_retry", Some(TaskState::Queued),          TaskState::Leased,  None),
        task_transition("t6", "task_retry", Some(TaskState::Leased),          TaskState::Running, None),
        task_transition("t7", "task_retry", Some(TaskState::Running),         TaskState::RetryableFailed, None),
    ]).await.unwrap();

    let after_second_fail = TaskReadModel::get(&store, &TaskId::new("task_retry"))
        .await.unwrap().unwrap();
    assert_eq!(after_second_fail.retry_count, 2, "second retryable failure = retry_count 2");
}
...
async fn list_by_parent_run_returns_tasks_with_correct_states() {
    ...
    store.append(&[
        ...
        evt("e_r", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new(sess),
            run_id: RunId::new("run_list"), parent_run_id: None,
            prompt_release_id: None, agent_role_id: None,
        })),
        // Task A: will complete.
        evt("e_ta", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: TaskId::new("task_lst_a"),
            parent_run_id: Some(RunId::new("run_list")),
            parent_task_id: None, prompt_release_id: None,
        })),
        // Task B: will fail.
        evt("e_tb", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: TaskId::new("task_lst_b"),
            parent_run_id: Some(RunId::new("run_list")),
            parent_task_id: None, prompt_release_id: None,
        })),
        // Task C: still queued.
        evt("e_tc", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: TaskId::new("task_lst_c"),
            parent_run_id: Some(RunId::new("run_list")),
            parent_task_id: None, prompt_release_id: None,
        })),
    ]).await.unwrap();
    ...
    store.append(&[
        task_transition("a1", "task_lst_a", Some(TaskState::Queued),  TaskState::Leased,    None),
        task_transition("a2", "task_lst_a", Some(TaskState::Leased),  TaskState::Running,   None),
        task_transition("a3", "task_lst_a", Some(TaskState::Running), TaskState::Completed, None),
    ]).await.unwrap();

    // Transition B to Failed.
    store.append(&[
        task_transition("b1", "task_lst_b", Some(TaskState::Queued),  TaskState::Leased,  None),
        task_transition("b2", "task_lst_b", Some(TaskState::Leased),  TaskState::Running, None),
        task_transition("b3", "task_lst_b", Some(TaskState::Running), TaskState::Failed,  Some(FailureClass::ExecutionError)),
    ]).await.unwrap();
    ...
    assert!(tasks.iter().all(|t| t.parent_run_id == Some(RunId::new("run_list"))));
    ...
    assert_eq!(find("task_lst_b").failure_class, Some(FailureClass::ExecutionError));
    assert_eq!(find("task_lst_c").state, TaskState::Queued, "C was never transitioned");
}
...
async fn list_by_parent_run_scoped_to_run() {
    ...
    store.append(&[
        ...
        evt("e_r1", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new(sess),
            run_id: RunId::new("run_iso_1"), parent_run_id: None,
            prompt_release_id: None, agent_role_id: None,
        })),
        evt("e_r2", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new(sess),
            run_id: RunId::new("run_iso_2"), parent_run_id: None,
            prompt_release_id: None, agent_role_id: None,
        })),
        evt("e_t1", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: TaskId::new("task_iso_1"),
            parent_run_id: Some(RunId::new("run_iso_1")),
            parent_task_id: None, prompt_release_id: None,
        })),
        evt("e_t2", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: TaskId::new("task_iso_2"),
...
async fn version_increments_on_every_transition() {
    ...
    store.append(&[task_transition("t1", "task_ver", Some(TaskState::Queued), TaskState::Leased, None)])
        .await.unwrap();
    check!(&store, 2, TaskState::Leased);

    store.append(&[task_transition("t2", "task_ver", Some(TaskState::Leased), TaskState::Running, None)])
        .await.unwrap();
    check!(&store, 3, TaskState::Running);

    store.append(&[task_transition("t3", "task_ver", Some(TaskState::Running), TaskState::WaitingApproval, None)])
        .await.unwrap();
    check!(&store, 4, TaskState::WaitingApproval);

    store.append(&[task_transition("t4", "task_ver", Some(TaskState::WaitingApproval), TaskState::Running, None)])
        .await.unwrap();
    check!(&store, 5, TaskState::Running);

    store.append(&[task_transition("t5", "task_ver", Some(TaskState::Running), TaskState::Completed, None)])
        .await.unwrap();
...
fn valid_task_transitions_per_domain() {
    // Core happy path.
    assert!(can_transition_task_state(TaskState::Queued,          TaskState::Leased));
    assert!(can_transition_task_state(TaskState::Leased,          TaskState::Running));
    assert!(can_transition_task_state(TaskState::Running,         TaskState::WaitingApproval));
    assert!(can_transition_task_state(TaskState::WaitingApproval, TaskState::Running));
    assert!(can_transition_task_state(TaskState::Running,         TaskState::Completed));

    // Failure paths.
    assert!(can_transition_task_state(TaskState::Running,         TaskState::Failed));
    assert!(can_transition_task_state(TaskState::WaitingApproval, TaskState::Failed));
    assert!(can_transition_task_state(TaskState::Running,         TaskState::RetryableFailed));
    assert!(can_transition_task_state(TaskState::RetryableFailed, TaskState::Queued));

    // Invalid transitions.
    assert!(!can_transition_task_state(TaskState::Completed, TaskState::Running));
    assert!(!can_transition_task_state(TaskState::Failed,    TaskState::Running));
    assert!(!can_transition_task_state(TaskState::Queued,    TaskState::Running), "must go via Leased");
    assert!(!can_transition_task_state(TaskState::Queued,    TaskState::Completed));
}
