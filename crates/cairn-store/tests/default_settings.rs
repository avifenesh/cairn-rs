...
use cairn_domain::{
    DefaultSettingCleared, DefaultSettingSet, EventEnvelope, EventId, EventSource, RuntimeEvent,
};
use cairn_store::{projections::DefaultsReadModel, EventLog, InMemoryStore};
...
fn clear(evt_id: &str, scope: Scope, scope_id: &str, key: &str) -> EventEnvelope<RuntimeEvent> {
    evt(
        evt_id,
...
async fn setting_cleared_removes_record() {
    ...
    store
        .append(&[clear("e2", Scope::Tenant, "t_clear", "timeout_ms")])
        .await
        .unwrap();
    ...
    assert!(
        after.is_none(),
        "setting must be absent after DefaultSettingCleared"
    );
}
...
async fn clearing_nonexistent_setting_is_noop() {
    ...
    store
        .append(&[clear("e1", Scope::Tenant, "t_noop", "ghost_key")])
        .await
        .unwrap();
...
async fn workspace_scope_isolated_from_tenant_scope() {
    ...
    assert!(
        none.is_none(),
        "workspace setting must not appear under tenant scope"
    );
}
...
async fn set_clear_set_cycle() {
    ...
    // Clear.
    store
        .append(&[clear("e2", Scope::Tenant, "t_cycle", "toggle")])
        .await
        .unwrap();
