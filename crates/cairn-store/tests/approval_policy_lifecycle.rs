...
use cairn_store::{
    projections::ApprovalPolicyReadModel,
...
fn create_policy(
    ...
    evt(evt_id, RuntimeEvent::ApprovalPolicyCreated(ApprovalPolicyCreated {
...
async fn approval_policy_created_stores_record() {
    ...
    store.append(&[create_policy(
        "e1", "policy_strict", "tenant_a", "Strict Review",
        2,
...
async fn timing_fields_persist_correctly() {
    ...
    store.append(&[create_policy(
        "e1", "policy_timed", "tenant_b", "Timed Policy",
        1,
...
async fn none_timing_fields_persist_as_none() {
    ...
    store.append(&[create_policy(
        "e1", "policy_manual", "tenant_c", "Manual Only",
        3,
...
async fn allowed_approver_roles_persist_correctly() {
    ...
    store.append(&[create_policy(
        "e1", "policy_roles", "tenant_d", "Roles Policy",
        2,
        vec![WorkspaceRole::Admin, WorkspaceRole::Owner],
        None, None, ts,
    )]).await.unwrap();
    ...
    assert!(policy.allowed_approver_roles.contains(&WorkspaceRole::Admin));
    assert!(policy.allowed_approver_roles.contains(&WorkspaceRole::Owner));
    assert!(!policy.allowed_approver_roles.contains(&WorkspaceRole::Member),
...
async fn attached_release_ids_starts_empty() {
    ...
    store.append(&[create_policy(
        "e1", "policy_attach", "tenant_e", "Attach Test",
        1, vec![WorkspaceRole::Admin], None, None, ts,
    )]).await.unwrap();
...
async fn attached_release_ids_can_be_updated() {
    ...
    store.append(&[create_policy(
        "e1", "policy_upd", "tenant_f", "Update Test",
        2, vec![WorkspaceRole::Admin, WorkspaceRole::Owner], None, None, ts,
    )]).await.unwrap();

    // Attach two releases.
    let ok1 = store.attach_release_to_policy(
        "policy_upd", PromptReleaseId::new("rel_v1"),
    );
    let ok2 = store.attach_release_to_policy(
        "policy_upd", PromptReleaseId::new("rel_v2"),
    );
    assert!(ok1, "attach_release_to_policy returns true for registered policy");
    assert!(ok2);

    let policy = ApprovalPolicyReadModel::get_policy(&store, "policy_upd")
        .await.unwrap().unwrap();
    assert_eq!(policy.attached_release_ids.len(), 2);
    assert!(policy.attached_release_ids.contains(&PromptReleaseId::new("rel_v1")));
    assert!(policy.attached_release_ids.contains(&PromptReleaseId::new("rel_v2")));
}
...
async fn attach_release_is_idempotent() {
    ...
    store.append(&[create_policy(
        "e1", "policy_idem", "tenant_g", "Idempotent",
        1, vec![WorkspaceRole::Admin], None, None, ts,
    )]).await.unwrap();
    ...
    let policy = ApprovalPolicyReadModel::get_policy(&store, "policy_idem")
        .await.unwrap().unwrap();
    assert_eq!(policy.attached_release_ids.len(), 1,
...
async fn list_by_tenant_returns_only_matching_tenant_policies() {
    ...
    store.append(&[
        create_policy("e1", "pol_t1_a", "t_iso_1", "Policy A", 1, vec![WorkspaceRole::Admin], None, None, ts),
        create_policy("e2", "pol_t1_b", "t_iso_1", "Policy B", 2, vec![WorkspaceRole::Owner], None, None, ts + 1),
        create_policy("e3", "pol_t2_a", "t_iso_2", "Policy C", 1, vec![WorkspaceRole::Admin], None, None, ts + 2),
    ]).await.unwrap();

    let t1_policies = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_iso_1"), 10, 0,
    ).await.unwrap();
    assert_eq!(t1_policies.len(), 2, "tenant_1 has 2 policies");
    assert!(t1_policies.iter().all(|p| p.tenant_id.as_str() == "t_iso_1"));
    ...
    assert!(!t1_ids.contains(&"pol_t2_a"), "t_iso_2 policy must not appear");

    let t2_policies = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_iso_2"), 10, 0,
    ).await.unwrap();
    assert_eq!(t2_policies.len(), 1);
    assert_eq!(t2_policies[0].policy_id, "pol_t2_a");

    // Unknown tenant returns empty.
    let empty = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_nobody"), 10, 0,
    ).await.unwrap();
...
async fn cross_tenant_policies_are_isolated() {
    ...
    store.append(&[
        create_policy("e1", "alpha_pol", "alpha_tenant", "Security Review",
            2, vec![WorkspaceRole::Admin], None, Some(86_400_000), ts),
        create_policy("e2", "beta_pol",  "beta_tenant",  "Security Review",
            1, vec![WorkspaceRole::Owner], Some(3_600_000), None, ts + 1),
    ]).await.unwrap();
    ...
    let alpha_list = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("alpha_tenant"), 10, 0,
    ).await.unwrap();
    assert_eq!(alpha_list.len(), 1);
    assert_eq!(alpha_list[0].policy_id, "alpha_pol");

    let beta_list = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("beta_tenant"), 10, 0,
    ).await.unwrap();
...
async fn list_by_tenant_sorted_by_policy_id() {
    ...
    store.append(&[
        create_policy("e1", "zzz_pol", "t_sort", "ZZZ", 1, vec![WorkspaceRole::Admin], None, None, ts),
        create_policy("e2", "aaa_pol", "t_sort", "AAA", 1, vec![WorkspaceRole::Admin], None, None, ts + 1),
        create_policy("e3", "mmm_pol", "t_sort", "MMM", 1, vec![WorkspaceRole::Admin], None, None, ts + 2),
    ]).await.unwrap();

    let policies = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_sort"), 10, 0,
    ).await.unwrap();
...
async fn list_by_tenant_respects_limit_and_offset() {
    ...
    for i in 0u32..4 {
        store.append(&[create_policy(
            ...
            "t_page",
            &format!("Policy {i}"),
            1, vec![WorkspaceRole::Admin],
            None, None,
            ts + i as u64,
        )]).await.unwrap();
    }

    let page1 = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_page"), 2, 0,
    ).await.unwrap();
    assert_eq!(page1.len(), 2);
    assert_eq!(page1[0].policy_id, "pol_pg_00");
    assert_eq!(page1[1].policy_id, "pol_pg_01");

    let page2 = ApprovalPolicyReadModel::list_by_tenant(
        &store, &TenantId::new("t_page"), 2, 2,
    ).await.unwrap();
...
async fn get_policy_returns_none_for_unknown_id() {
    let store = InMemoryStore::new();
    let result = ApprovalPolicyReadModel::get_policy(&store, "nonexistent").await.unwrap();
