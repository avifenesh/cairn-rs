...
use cairn_domain::commercial::{DefaultFeatureGate, EntitlementSet, FeatureGate, FeatureGateResult, ProductTier};
...
use cairn_store::{
    ...
    projections::{ApprovalReadModel, PromptReleaseReadModel, RouteDecisionReadModel, RunReadModel},
...
fn project(tenant: &str) -> ProjectKey { ProjectKey::new(tenant, "ws", "p") }
...
async fn rfc002_causation_id_idempotency() {
    ...
    let first = EventEnvelope::for_runtime_event(
        EventId::new("evt_1"), EventSource::Runtime,
        RuntimeEvent::SessionCreated(SessionCreated { project: project("t1"), session_id: SessionId::new("s1") }),
    ).with_causation_id(CommandId::new("cmd_1"));

    let pos = EventLog::append(store.as_ref(), &[first]).await.unwrap()[0];
    let found = EventLog::find_by_causation_id(store.as_ref(), "cmd_1").await.unwrap();

    // MUST: causation_id lookup returns the exact position of the first application.
    assert_eq!(found, Some(pos), "RFC 002: causation_id must resolve to the appended position");
    // MUST: re-delivery is idempotent — same position, no second append.
    assert_eq!(found.unwrap(), pos);
    let stream = EventLog::read_stream(store.as_ref(), None, 100).await.unwrap();
    assert_eq!(stream.len(), 1, "RFC 002: exactly 1 event after idempotent re-delivery");
}
...
async fn rfc005_approval_blocks_run_progression() {
    ...
    store.append(&[
        ev("sess", RuntimeEvent::SessionCreated(SessionCreated { project: project("t1"), session_id: SessionId::new("s1") })),
        ev("run",  RuntimeEvent::RunCreated(RunCreated { project: project("t1"), session_id: SessionId::new("s1"), run_id: RunId::new("r1"), parent_run_id: None, prompt_release_id: None, agent_role_id: None })),
        ev("wait", RuntimeEvent::RunStateChanged(RunStateChanged { project: project("t1"), run_id: RunId::new("r1"), transition: StateTransition { from: Some(RunState::Running), to: RunState::WaitingApproval }, failure_class: None, pause_reason: None, resume_trigger: None })),
        ev("appr", RuntimeEvent::ApprovalRequested(ApprovalRequested { project: project("t1"), approval_id: ApprovalId::new("a1"), run_id: Some(RunId::new("r1")), task_id: None, requirement: ApprovalRequirement::Required })),
    ]).await.unwrap();

    // MUST: run is blocked in WaitingApproval.
    let run = RunReadModel::get(store.as_ref(), &RunId::new("r1")).await.unwrap().unwrap();
    assert_eq!(run.state, RunState::WaitingApproval, "RFC 005: run must be blocked in WaitingApproval");

    // MUST: approval is visible in the operator queue.
    let pending = ApprovalReadModel::list_pending(store.as_ref(), &project("t1"), 10, 0).await.unwrap();
    assert_eq!(pending.len(), 1, "RFC 005: blocked run must have 1 pending approval");
...
async fn rfc006_prompt_release_draft_to_active() {
    ...
    assert_eq!(active.state, "active", "RFC 006: release must be 'active' after PromptReleaseTransitioned");
}
...
async fn rfc008_cross_tenant_isolation() {
    ...
    store.append(&[
        ev("s_a", RuntimeEvent::SessionCreated(SessionCreated { project: project("tenant_a"), session_id: SessionId::new("sa") })),
        ev("r_a", RuntimeEvent::RunCreated(RunCreated { project: project("tenant_a"), session_id: SessionId::new("sa"), run_id: RunId::new("run_a"), parent_run_id: None, prompt_release_id: None, agent_role_id: None })),
    ]).await.unwrap();

    // MUST: tenant_a's run is NOT visible when queried as tenant_b.
    let run_via_b = RunReadModel::get(store.as_ref(), &RunId::new("run_a")).await.unwrap().unwrap();
    assert_ne!(run_via_b.project.tenant_id.as_str(), "tenant_b",
        "RFC 008: run owned by tenant_a must not be returned as tenant_b's data");

    // MUST: a non-existent run for tenant_b returns None.
    let no_run = RunReadModel::get(store.as_ref(), &RunId::new("run_b")).await.unwrap();
    assert!(no_run.is_none(), "RFC 008: tenant_b has no runs — read must return None");
}
...
async fn rfc009_route_decision_persisted_with_fallback_flag() {
    ...
    store.append(&[ev("rd", RuntimeEvent::RouteDecisionMade(RouteDecisionMade {
        project: project("t1"),
        route_decision_id: RouteDecisionId::new("rd_1"),
        operation_kind: OperationKind::Generate,
        selected_provider_binding_id: Some(cairn_domain::ProviderBindingId::new("binding_fallback")),
    ...
    let decision = RouteDecisionReadModel::get(store.as_ref(), &RouteDecisionId::new("rd_1")).await.unwrap().unwrap();

    // MUST: fallback_used is durable — operators can see when primary failed.
    assert!(decision.fallback_used, "RFC 009: fallback_used=true MUST be persisted and readable");
