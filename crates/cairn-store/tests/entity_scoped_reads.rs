...
async fn read_by_entity_returns_only_matching_events() {
    ...
    // Session events.
    store.append(&[
        ev("evt_sess_a_created",
            RuntimeEvent::SessionCreated(SessionCreated {
                project: project(),
                session_id: session_id("a"),
            })),
        ev("evt_sess_b_created",
            RuntimeEvent::SessionCreated(SessionCreated {
                project: project(),
    ...
    assert_eq!(sess_a_events.len(), 1, "session_a must have exactly 1 event");
    assert!(matches!(
    ...
    let run1_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Run(run_id("1")),
        None,
        100,
    ).await.unwrap();
    assert_eq!(run1_events.len(), 2, "run_1 must have Created + StateChanged = 2 events");
    assert!(run1_events.iter().all(|e| {
    ...
    let run2_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Run(run_id("2")),
        None,
        100,
    ).await.unwrap();
    assert_eq!(run2_events.len(), 1, "run_2 must have only 1 event");
    assert!(!run2_events.iter().any(|e| {
    ...
    let task1_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Task(task_id("1")),
        None,
        100,
    ).await.unwrap();
    assert_eq!(task1_events.len(), 2, "task_1 must have Created + StateChanged = 2 events");
}
...
async fn cursor_based_pagination_skips_prior_events() {
    ...
    let page1 = EventLog::read_by_entity(
        store.as_ref(),
    ...
    assert_eq!(page2.len(), 3, "page 2 must return the remaining 3 events after cursor");
...
async fn head_position_reflects_latest_append() {
    ...
    assert!(head_empty.is_none(), "empty store must report head_position = None");

    // Append first event.
    let pos1 = EventLog::append(
        store.as_ref(),
        &[ev("evt_head_1", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(),
            session_id: session_id("head1"),
        }))],
    ).await.unwrap()[0];

    let head1 = EventLog::head_position(store.as_ref()).await.unwrap().unwrap();
    assert_eq!(head1, pos1, "head_position must equal position of first appended event");

    // Append two more events.
    let mut positions = EventLog::append(
        store.as_ref(),
        &[
            ev("evt_head_2", RuntimeEvent::SessionCreated(SessionCreated {
                project: project(),
                session_id: session_id("head2"),
            })),
            ev("evt_head_3", RuntimeEvent::SessionCreated(SessionCreated {
                project: project(),
    ...
    let head3 = EventLog::head_position(store.as_ref()).await.unwrap().unwrap();
    assert_eq!(
        head3, last_pos,
...
async fn read_stream_returns_all_events_in_order() {
    ...
    store.append(&[
        ev("evt_s1", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: session_id("s1"),
        })),
        ev("evt_r1", RuntimeEvent::RunCreated(RunCreated {
            project: project(),
        ...
        ev("evt_t1", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(),
            task_id: task_id("t1"),
        ...
        ev("evt_s2", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: session_id("s2"),
        })),
    ]).await.unwrap();
    ...
    let tail = EventLog::read_stream(store.as_ref(), Some(cursor), 100).await.unwrap();
    assert_eq!(tail.len(), 2, "read_stream after cursor must return only events after it");
    assert!(tail.iter().all(|e| e.position > cursor));

    // read_stream with limit truncates the result set.
    let limited = EventLog::read_stream(store.as_ref(), None, 2).await.unwrap();
    assert_eq!(limited.len(), 2, "read_stream with limit=2 must return exactly 2 events");
}
...
async fn entity_ref_matching_all_types() {
    ...
    store.append(&[
        // Session
        ev("evt_sess_x", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(),
            session_id: session_id("x"),
        })),
        // Run
        ev("evt_run_x", RuntimeEvent::RunCreated(RunCreated {
            project: project(),
            session_id: session_id("x"),
            run_id: run_id("x"),
            parent_run_id: None,
        ...
        // Approval
        ev("evt_appr_x", RuntimeEvent::ApprovalRequested(ApprovalRequested {
            project: project(),
        ...
        // Checkpoint
        ev("evt_ckpt_x", RuntimeEvent::CheckpointRecorded(CheckpointRecorded {
            project: project(),
            run_id: run_id("x"),
    ...
    let sess_events = EventLog::read_by_entity(
        store.as_ref(), &EntityRef::Session(session_id("x")), None, 100
    ).await.unwrap();
    assert!(
        !sess_events.iter().any(|e| matches!(&e.envelope.payload, RuntimeEvent::RunCreated(_))),
        "Session entity read must not include Run events"
    );

    let run_events = EventLog::read_by_entity(
        store.as_ref(), &EntityRef::Run(run_id("x")), None, 100
    ).await.unwrap();
    assert!(
        !run_events.iter().any(|e| matches!(&e.envelope.payload, RuntimeEvent::TaskCreated(_))),
        "Run entity read must not include Task events"
    );

    // Non-existent entity returns empty.
    let missing = EventLog::read_by_entity(
        store.as_ref(), &EntityRef::Session(session_id("does_not_exist")), None, 100
    ).await.unwrap();
    assert!(missing.is_empty(), "non-existent entity must return empty event list");
}
...
async fn same_type_different_id_events_are_independent() {
    ...
    for n in ["alpha", "beta"] {
        store.append(&[
            ev(&format!("evt_{n}_created"), RuntimeEvent::SessionCreated(SessionCreated {
                project: project(),
    ...
    for i in 1..=3u8 {
        store.append(&[
            ev(&format!("evt_run_alpha_{i}"), RuntimeEvent::RunCreated(RunCreated {
                project: project(),
