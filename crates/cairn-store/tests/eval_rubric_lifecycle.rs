...
use cairn_domain::{
    EvalBaselineLocked, EvalBaselineSet, EvalRubricCreated, EventEnvelope, EventId,
    EventSource, RuntimeEvent, TenantId,
};
use cairn_store::{
...
fn create_rubric(evt_id: &str, rubric_id: &str, name: &str, ts: u64) -> EventEnvelope<RuntimeEvent> {
    evt(evt_id, RuntimeEvent::EvalRubricCreated(EvalRubricCreated {
...
fn set_baseline(evt_id: &str, baseline_id: &str, metric: &str, value: &str, ts: u64)
    -> EventEnvelope<RuntimeEvent>
{
    evt(evt_id, RuntimeEvent::EvalBaselineSet(EvalBaselineSet {
...
fn lock_baseline(evt_id: &str, baseline_id: &str, ts: u64) -> EventEnvelope<RuntimeEvent> {
    evt(evt_id, RuntimeEvent::EvalBaselineLocked(EvalBaselineLocked {
        baseline_id:  baseline_id.to_owned(),
        locked_at_ms: ts,
    }))
}
...
async fn eval_rubric_created_stores_record() {
    ...
    store.append(&[create_rubric("e1", "rubric_001", "Accuracy Rubric", ts)])
        .await.unwrap();
    ...
    assert!(rubric.dimensions.is_empty(), "new rubric has no dimensions from event");
}
...
async fn get_rubric_returns_none_for_unknown_id() {
    let store = InMemoryStore::new();
    assert!(EvalRubricReadModel::get_rubric(&store, "ghost").await.unwrap().is_none());
}
...
async fn eval_rubric_created_is_idempotent() {
    ...
    let rubric = EvalRubricReadModel::get_rubric(&store, "rubric_idem")
        .await.unwrap().unwrap();
    assert_eq!(rubric.name, "Version 1",
...
async fn eval_baseline_set_stores_record() {
    ...
    store.append(&[set_baseline("e1", "baseline_001", "task_success_rate", "0.92", ts)])
        .await.unwrap();
...
async fn eval_baseline_set_updates_metric() {
    ...
    assert!(baseline.name.contains("latency_p50_ms"),
...
async fn eval_baseline_locked_sets_locked_flag() {
    ...
    store.append(&[
        set_baseline("e1", "bl_lock", "task_success_rate", "0.90", ts),
    ]).await.unwrap();
    ...
    store.append(&[lock_baseline("e2", "bl_lock", ts + 1_000)])
        .await.unwrap();
...
async fn locking_unknown_baseline_is_noop() {
    let store = InMemoryStore::new();
    store.append(&[lock_baseline("e1", "ghost_baseline", now_ms())])
        .await.unwrap();
    let result = EvalBaselineReadModel::get_baseline(&store, "ghost_baseline")
        .await.unwrap();
    assert!(result.is_none(), "locking non-existent baseline must not create a record");
}
...
async fn multiple_rubrics_tracked_independently() {
    ...
    for (id, name) in [("rubric_a", "Safety Rubric"), ("rubric_b", "Quality Rubric"), ("rubric_c", "Latency Rubric")] {
        let r = EvalRubricReadModel::get_rubric(&store, id).await.unwrap().unwrap();
...
async fn multiple_baselines_tracked_independently() {
    ...
    let prod    = EvalBaselineReadModel::get_baseline(&store, "bl_prod").await.unwrap().unwrap();
    let staging = EvalBaselineReadModel::get_baseline(&store, "bl_staging").await.unwrap().unwrap();
...
async fn list_by_tenant_sentinel_returns_all_baselines() {
    ...
    let locked_one = baselines.iter().find(|b| b.baseline_id == "bl_list_a").unwrap();
    let open_one   = baselines.iter().find(|b| b.baseline_id == "bl_list_b").unwrap();
...
async fn list_by_tenant_rubrics_pagination() {
    ...
    for i in 0u64..4 {
        store.append(&[create_rubric(
            &format!("e{i}"), &format!("rub_pg_{i:02}"),
            &format!("Rubric {i}"), ts + i,
        )]).await.unwrap();
    }
