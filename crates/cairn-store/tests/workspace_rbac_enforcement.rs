...
fn role_hierarchy_owner_satisfies_all_levels() {
    assert!(
        WorkspaceRole::Owner.has_at_least(WorkspaceRole::Owner),
        "Owner ≥ Owner"
    );
    assert!(
        WorkspaceRole::Owner.has_at_least(WorkspaceRole::Admin),
        "Owner ≥ Admin"
    );
    assert!(
        WorkspaceRole::Owner.has_at_least(WorkspaceRole::Member),
        "Owner ≥ Member"
    );
    assert!(
        WorkspaceRole::Owner.has_at_least(WorkspaceRole::Viewer),
        "Owner ≥ Viewer"
    );
}
...
fn role_hierarchy_admin_satisfies_admin_and_below() {
    assert!(
        !WorkspaceRole::Admin.has_at_least(WorkspaceRole::Owner),
        "Admin < Owner"
    );
    assert!(
        WorkspaceRole::Admin.has_at_least(WorkspaceRole::Admin),
        "Admin ≥ Admin"
    );
    assert!(
        WorkspaceRole::Admin.has_at_least(WorkspaceRole::Member),
        "Admin ≥ Member"
    );
    assert!(
        WorkspaceRole::Admin.has_at_least(WorkspaceRole::Viewer),
        "Admin ≥ Viewer"
    );
}
...
fn role_hierarchy_member_satisfies_member_and_viewer() {
    assert!(
        !WorkspaceRole::Member.has_at_least(WorkspaceRole::Owner),
        "Member < Owner"
    );
    assert!(
        !WorkspaceRole::Member.has_at_least(WorkspaceRole::Admin),
        "Member < Admin"
    );
    assert!(
        WorkspaceRole::Member.has_at_least(WorkspaceRole::Member),
        "Member ≥ Member"
    );
    assert!(
        WorkspaceRole::Member.has_at_least(WorkspaceRole::Viewer),
        "Member ≥ Viewer"
    );
}
...
fn role_hierarchy_viewer_satisfies_only_viewer() {
    assert!(
        !WorkspaceRole::Viewer.has_at_least(WorkspaceRole::Owner),
        "Viewer < Owner"
    );
    assert!(
        !WorkspaceRole::Viewer.has_at_least(WorkspaceRole::Admin),
        "Viewer < Admin"
    );
    assert!(
        !WorkspaceRole::Viewer.has_at_least(WorkspaceRole::Member),
        "Viewer < Member"
    );
    assert!(
        WorkspaceRole::Viewer.has_at_least(WorkspaceRole::Viewer),
        "Viewer ≥ Viewer"
    );
}
...
async fn member_removed_event_removes_access() {
    ...
    let alice = WorkspaceMembershipReadModel::get_member(&store, &wkey("t_rm", "ws_rm"), "alice")
        .await
        .unwrap();
...
async fn removing_all_members_leaves_workspace_empty() {
    ...
    assert!(
        members.is_empty(),
        "all members removed — workspace has no members"
    );
}
...
async fn cross_workspace_member_isolation() {
    ...
    let carol_alpha =
        WorkspaceMembershipReadModel::get_member(&store, &wkey("t_iso", "ws_alpha"), "carol")
            .await
            .unwrap()
            .unwrap();
    ...
    let carol_beta =
        WorkspaceMembershipReadModel::get_member(&store, &wkey("t_iso", "ws_beta"), "carol")
            .await
            .unwrap()
            .unwrap();
    ...
    assert!(
        carol_alpha.role.has_at_least(carol_beta.role),
        "Admin satisfies Viewer minimum"
    );
    assert!(
        !carol_beta.role.has_at_least(carol_alpha.role),
...
fn workspace_role_serializes_to_snake_case() {
    ...
    assert_eq!(
        serde_json::to_value(WorkspaceRole::Member).unwrap(),
        "member"
    );
    assert_eq!(
        serde_json::to_value(WorkspaceRole::Viewer).unwrap(),
        "viewer"
    );
}
...
async fn re_adding_member_upgrades_role_without_duplicate() {
    ...
    let before = WorkspaceMembershipReadModel::get_member(&store, &wkey("t_up", "ws_up"), "eve")
        .await
        .unwrap()
        .unwrap();
...
async fn members_are_scoped_to_tenant_workspace_pair() {
    ...
    let alice =
        WorkspaceMembershipReadModel::get_member(&store, &wkey("tenant_x", "ws_shared"), "alice")
            .await
            .unwrap()
            .unwrap();
    ...
    let bob =
        WorkspaceMembershipReadModel::get_member(&store, &wkey("tenant_y", "ws_shared"), "bob")
            .await
            .unwrap()
            .unwrap();
