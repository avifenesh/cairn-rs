...
fn run_id(n: &str) -> RunId {
    RunId::new(format!("run_rec_{n}"))
}
fn task_id(n: &str) -> TaskId {
    TaskId::new(format!("task_rec_{n}"))
}
fn sess_id(n: &str) -> SessionId {
    SessionId::new(format!("sess_rec_{n}"))
}
...
async fn recovery_attempted_is_stored_in_event_log() {
    ...
    assert_eq!(
        rec_events.len(),
        1,
        "one recovery event must be stored for the run"
    );
...
async fn recovery_completed_success_marks_recovered() {
    ...
    assert_eq!(
        rec_events.len(),
        2,
        "RecoveryAttempted + RecoveryCompleted must both be stored"
    );

    // Verify completion is marked as successful.
    let completed = rec_events
        .iter()
        .find(|e| matches!(&e.envelope.payload, RuntimeEvent::RecoveryCompleted(c) if c.recovered));
    assert!(
        completed.is_some(),
        "RecoveryCompleted(recovered=true) must be stored"
    );
...
async fn recovery_escalated_stored_with_all_fields() {
    ...
    let escalation = rec_events
        .iter()
        .find(|e| matches!(&e.envelope.payload, RuntimeEvent::RecoveryEscalated(_)))
    ...
    if let RuntimeEvent::RecoveryEscalated(esc) = &escalation.envelope.payload {
        ...
        assert_eq!(
            esc.escalated_at_ms, 99_000,
            "escalated_at_ms must be preserved"
        );
        assert!(
            esc.reason.contains("operator intervention"),
            "reason must explain the escalation"
        );
    ...
    let failed_count = rec_events
        .iter()
        .filter(
            |e| matches!(&e.envelope.payload, RuntimeEvent::RecoveryCompleted(c) if !c.recovered),
        )
        .count();
    assert_eq!(
        failed_count, 3,
        "exactly 3 RecoveryCompleted(recovered=false) events"
    );
}
...
async fn escalation_chain_is_queryable_in_causal_order() {
    ...
    assert_eq!(
        rec_events.len(),
        3,
        "attempt + failed_complete + escalated = 3 events"
    );

    // (7) Events must appear in strict causal (position) order.
    assert!(
        matches!(
            &rec_events[0].envelope.payload,
    ...
    assert!(
        rec_events[1].position < rec_events[2].position,
...
async fn recovery_completed_false_is_distinct_from_success() {
    ...
    let has_failure = rec_events
        .iter()
        .any(|e| matches!(&e.envelope.payload, RuntimeEvent::RecoveryCompleted(c) if !c.recovered));
    let has_success = rec_events
        .iter()
        .any(|e| matches!(&e.envelope.payload, RuntimeEvent::RecoveryCompleted(c) if c.recovered));
    ...
    assert!(
        !has_success,
        "no successful completion must exist for this run"
    );
}
...
async fn recovery_events_are_isolated_by_run() {
    ...
    assert_eq!(
        a_events.len(),
        1,
        "run_a must see only its 1 recovery event"
    );
