...
use cairn_domain::{
    ...
    PromptReleaseCreated, PromptReleaseId, PromptVersionCreated, PromptVersionId, RuntimeEvent,
...
fn proj_a() -> ProjectKey {
    project("t_scope", "w_scope", "proj_a")
}
fn proj_b() -> ProjectKey {
    project("t_scope", "w_scope", "proj_b")
}
...
async fn list_by_project_returns_only_matching_project_assets() {
    ...
    let a_ids: Vec<_> = a_assets
        .iter()
        .map(|a| a.prompt_asset_id.as_str())
        .collect();
    assert!(a_ids.contains(&"asset_a1"));
    assert!(a_ids.contains(&"asset_a2"));
    assert!(
        !a_ids.contains(&"asset_b1"),
        "proj_b asset must not appear in proj_a list"
    );
    ...
    let empty = PromptAssetReadModel::list_by_project(
        &store,
        &project("t_scope", "w_scope", "proj_c"),
        10,
        0,
    )
    .await
...
async fn same_asset_name_allowed_in_different_projects() {
    ...
    assert_ne!(
        rec_a.prompt_asset_id, rec_b.prompt_asset_id,
...
async fn get_returns_correct_asset_regardless_of_project() {
    ...
    let a = PromptAssetReadModel::get(&store, &PromptAssetId::new("get_a"))
        .await
        .unwrap()
        .expect("get_a must be retrievable");
    ...
    let b = PromptAssetReadModel::get(&store, &PromptAssetId::new("get_b"))
        .await
        .unwrap()
        .expect("get_b must be retrievable");
...
async fn list_by_project_ordered_by_created_at() {
    ...
    assert_eq!(
        assets[0].prompt_asset_id.as_str(),
        "asset_ord_a",
        "oldest first"
    );
    assert_eq!(assets[1].prompt_asset_id.as_str(), "asset_ord_b");
    assert_eq!(
        assets[2].prompt_asset_id.as_str(),
        "asset_ord_c",
        "newest last"
    );
}
...
async fn version_carries_parent_asset_project() {
    ...
    let va_versions =
        PromptVersionReadModel::list_by_asset(&store, &PromptAssetId::new("ast_va"), 10, 0)
            .await
            .unwrap();
    ...
    let vb_versions =
        PromptVersionReadModel::list_by_asset(&store, &PromptAssetId::new("ast_vb"), 10, 0)
            .await
            .unwrap();
