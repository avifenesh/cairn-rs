...
fn session(n: &str) -> SessionId { SessionId::new(format!("sess_msg_{n}")) }
fn run(n: &str)     -> RunId     { RunId::new(format!("run_msg_{n}")) }
...
fn msg_event(
    ...
    ev(id, RuntimeEvent::UserMessageAppended(UserMessageAppended {
        project: project(),
...
async fn seed_session_run(store: &Arc<InMemoryStore>, n: &str) {
    store.append(&[
        ev(&format!("evt_sess_{n}"), RuntimeEvent::SessionCreated(SessionCreated {
            project: project(),
            session_id: session(n),
        })),
        ev(&format!("evt_run_{n}"), RuntimeEvent::RunCreated(RunCreated {
            project: project(),
            session_id: session(n),
            run_id: run(n),
            parent_run_id: None,
...
async fn user_message_appended_is_stored_in_session_context() {
    ...
    assert!(msg.is_some(), "UserMessageAppended must appear in the event log");
...
async fn multiple_messages_stored_and_ordered_by_sequence() {
    ...
    let messages: Vec<&UserMessageAppended> = run_events.iter()
        .filter_map(|e| {
    ...
    assert_eq!(messages.len(), 4, "all 4 messages must be present in the run's event stream");
...
async fn messages_from_different_sessions_are_isolated() {
    ...
    let x_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Run(run("x")),
        None, 100,
    ).await.unwrap();
    let x_msgs: Vec<_> = x_events.iter()
    ...
    let y_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Run(run("y")),
        None, 100,
    ).await.unwrap();
    let y_msgs: Vec<_> = y_events.iter()
        .filter(|e| matches!(&e.envelope.payload, RuntimeEvent::UserMessageAppended(_)))
        .collect();
...
async fn message_content_round_trips_without_loss() {
    ...
    let run_events = EventLog::read_by_entity(
        store.as_ref(),
        &EntityRef::Run(run("rt")),
        None, 100,
    ).await.unwrap();

    let recovered: Vec<&UserMessageAppended> = run_events.iter()
        .filter_map(|e| {
            if let RuntimeEvent::UserMessageAppended(m) = &e.envelope.payload {
                Some(m)
            } else {
    ...
    assert_eq!(recovered.len(), test_messages.len(),
        "all {} messages must be recoverable", test_messages.len());

    // Content of each message must be exactly preserved.
    for (i, (_, expected_content)) in test_messages.iter().enumerate() {
        assert_eq!(
            recovered[i].content, *expected_content,
...
async fn user_messages_support_cursor_based_pagination() {
    ...
    let page2_msgs: Vec<_> = page2.iter()
        .filter(|e| matches!(&e.envelope.payload, RuntimeEvent::UserMessageAppended(_)))
        .collect();
    assert_eq!(page2_msgs.len(), 4, "page 2 must return the 4 remaining messages");
    ...
    assert!(p1_positions.is_disjoint(&p2_positions), "pages must not overlap");
}
