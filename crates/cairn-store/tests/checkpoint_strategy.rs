...
async fn strategy_update_replaces_previous() {
    ...
    // Initial strategy.
    store
        .append(&[set_strategy("e1", "strat_v1", run_id, "version 1", ts)])
        .await
        .unwrap();
        ...
        .append(&[
            evt(
                ...
                RuntimeEvent::SessionCreated(SessionCreated {
                    project: project(),
                    session_id: SessionId::new("sess_disp"),
                }),
            ),
    ...
    store
        .append(&[
            evt(
                ...
                RuntimeEvent::SessionCreated(SessionCreated {
                    project: project(),
                    session_id: SessionId::new("sess_sup"),
                }),
            ),
...
async fn latest_and_superseded_coexist_in_same_run() {
    ...
    store
        .append(&[
            evt(
                ...
                RuntimeEvent::SessionCreated(SessionCreated {
                    project: project(),
                    session_id: SessionId::new("sess_coex"),
                }),
            ),
    ...
    let cp_a = CheckpointReadModel::get(&store, &CheckpointId::new("cp_a"))
        .await
        .unwrap()
        .unwrap();
    let cp_b = CheckpointReadModel::get(&store, &CheckpointId::new("cp_b"))
        .await
        .unwrap()
        .unwrap();
    let cp_c = CheckpointReadModel::get(&store, &CheckpointId::new("cp_c"))
        .await
        .unwrap()
        .unwrap();
...
async fn checkpoint_restored_does_not_affect_strategy() {
    ...
    store
        .append(&[
            evt(
                ...
                RuntimeEvent::SessionCreated(SessionCreated {
                    project: project(),
                    session_id: SessionId::new("sess_restore"),
                }),
            ),
            record_checkpoint(
                "e1",
    ...
    let strat = CheckpointStrategyReadModel::get_by_run(&store, &RunId::new("run_restore"))
        ...
        .unwrap();
    assert_eq!(
        strat.strategy_id, "strat_restore",
