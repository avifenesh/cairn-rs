...
use cairn_domain::workers::{
    ExternalWorkerOutcome, ExternalWorkerReport,
};
use cairn_domain::{
...
use cairn_store::{
    projections::ExternalWorkerReadModel,
...
async fn worker_registered_stores_record_with_active_status() {
    ...
    assert!(!record.health.is_alive,
        "freshly registered worker has not yet heartbeated");
    assert_eq!(record.health.last_heartbeat_ms, 0,
...
async fn worker_reported_sets_health_alive_and_heartbeat() {
    ...
    assert!(after.health.is_alive, "health.is_alive=true after heartbeat");
    assert_eq!(after.health.last_heartbeat_ms, ts + 5_000,
        "last_heartbeat_ms must reflect the reported_at_ms");
    assert_eq!(after.current_task_id, Some(TaskId::new("task_001")),
...
async fn multiple_heartbeats_advance_last_heartbeat_ms() {
    ...
    store.append(&[register("e1", "worker_mhb", "t_mhb", "Multi-HB Bot", ts)])
        .await.unwrap();

    for i in 1u64..=3 {
        store.append(&[heartbeat("e_hb", "worker_mhb", "t_mhb", "task_x", ts + i * 10_000)])
            .await.unwrap();

        let r = ExternalWorkerReadModel::get(&store, &WorkerId::new("worker_mhb"))
            .await.unwrap().unwrap();
        assert_eq!(r.health.last_heartbeat_ms, ts + i * 10_000,
...
async fn heartbeat_with_outcome_clears_current_task() {
    ...
    store
        .append(&[heartbeat_with_outcome("e3", "worker_fin", "t_fin", "task_final", ts + 5_000)])
        .await
        .unwrap();
    ...
    assert!(finished.health.is_alive, "health.is_alive remains true even after terminal report");
}
...
async fn worker_suspended_sets_suspended_status() {
    ...
    store
        .append(&[
            ...
            suspend("e3", "worker_sus", "t_sus", Some("exceeded error threshold"), ts + 2_000),
        ])
        .await
        .unwrap();
...
async fn full_lifecycle_register_heartbeat_suspend_reactivate() {
    ...
    // 1. Register.
    store.append(&[register("e1", "worker_full", "t_full", "Full Lifecycle Bot", ts)])
        .await.unwrap();
    let r = ExternalWorkerReadModel::get(&store, &WorkerId::new("worker_full")).await.unwrap().unwrap();
    assert_eq!(r.status, "active");
    assert!(!r.health.is_alive);

    // 2. Heartbeat.
    store.append(&[heartbeat("e2", "worker_full", "t_full", "task_lc_1", ts + 1_000)])
        .await.unwrap();
    let r = ExternalWorkerReadModel::get(&store, &WorkerId::new("worker_full")).await.unwrap().unwrap();
    assert_eq!(r.status, "active");
    assert!(r.health.is_alive);
    ...
    // 3. Suspend.
    store.append(&[suspend("e3", "worker_full", "t_full", Some("maintenance window"), ts + 2_000)])
        .await.unwrap();
    let r = ExternalWorkerReadModel::get(&store, &WorkerId::new("worker_full")).await.unwrap().unwrap();
    assert_eq!(r.status, "suspended");

    // 4. Reactivate.
    store.append(&[reactivate("e4", "worker_full", "t_full", ts + 3_000)])
        .await.unwrap();
    let r = ExternalWorkerReadModel::get(&store, &WorkerId::new("worker_full")).await.unwrap().unwrap();
    assert_eq!(r.status, "active");
...
async fn list_by_tenant_returns_only_tenant_workers() {
    ...
    let t1_workers = ExternalWorkerReadModel::list_by_tenant(
        &store, &TenantId::new("tenant_one"), 10, 0,
    ).await.unwrap();
    assert_eq!(t1_workers.len(), 2, "tenant_one has 2 workers");
    assert!(t1_workers.iter().all(|w| w.tenant_id.as_str() == "tenant_one"));
    ...
    assert!(!t1_ids.contains(&"w_t2_a"), "tenant_two worker must not appear in tenant_one list");

    let t2_workers = ExternalWorkerReadModel::list_by_tenant(
    ...
    let empty = ExternalWorkerReadModel::list_by_tenant(
        &store, &TenantId::new("tenant_unknown"), 10, 0,
    ).await.unwrap();
...
async fn list_by_tenant_respects_limit_and_offset() {
    ...
    let all = ExternalWorkerReadModel::list_by_tenant(
        &store, &TenantId::new("t_page"), 10, 0,
    ).await.unwrap();
    assert_eq!(all.len(), 4);
    ...
    let page2 = ExternalWorkerReadModel::list_by_tenant(
        &store, &TenantId::new("t_page"), 10, 3,
    ).await.unwrap();
