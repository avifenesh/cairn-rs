...
use cairn_domain::{
    ...
    RouteAttemptId, RouteDecisionId, RouteDecisionMade, RunId, RuntimeEvent, SessionId,
...
async fn provider_call_completed_stores_all_audit_fields() {
    ...
    store
        .append(&[completed_call(
            "e1", "call_001", "rd_001", "ra_001",
            "gpt-4o", Some("run_1"), Some("sess_1"),
    ...
    assert_eq!(record.cost_micros, Some(8_500), "cost_micros must be stored");
}
...
async fn provider_call_status_variants_persist() {
    ...
    store
        .append(&[
            completed_call("e1", "call_ok",  "rd_ok",  "ra_1", "gpt-4o", None, None,
                ProviderCallStatus::Succeeded, Some(100), Some(256), Some(128), Some(5_000), ts),
            completed_call("e2", "call_fail", "rd_fail", "ra_2", "gpt-4o-mini", None, None,
                ProviderCallStatus::Failed, None, None, None, None, ts + 1),
            completed_call("e3", "call_cancel", "rd_cancel", "ra_3", "claude-haiku", None, None,
                ProviderCallStatus::Cancelled, None, None, None, None, ts + 2),
        ])
        .await
        .unwrap();

    let ok = ProviderCallReadModel::get(&store, &ProviderCallId::new("call_ok")).await.unwrap().unwrap();
    assert_eq!(ok.status, ProviderCallStatus::Succeeded);
    assert!(ok.cost_micros.is_some());

    let fail = ProviderCallReadModel::get(&store, &ProviderCallId::new("call_fail")).await.unwrap().unwrap();
    assert_eq!(fail.status, ProviderCallStatus::Failed);
    assert!(fail.cost_micros.is_none(), "failed call has no cost");

    let cancel = ProviderCallReadModel::get(&store, &ProviderCallId::new("call_cancel")).await.unwrap().unwrap();
...
async fn list_by_decision_returns_all_calls_for_decision() {
    ...
    store
        .append(&[
            completed_call("e1", "call_d1_a", "rd_run1", "ra_1a", "gpt-4o",
                Some("run_1"), None, ProviderCallStatus::Failed, Some(30_000), None, None, None, ts),
            completed_call("e2", "call_d1_b", "rd_run1", "ra_1b", "gpt-4o",
                Some("run_1"), None, ProviderCallStatus::Succeeded, Some(145), Some(512), Some(256), Some(8_500), ts + 1),
            // Different decision (different run).
            completed_call("e3", "call_d2_a", "rd_run2", "ra_2a", "gpt-4o-mini",
                Some("run_2"), None, ProviderCallStatus::Succeeded, Some(80), Some(200), Some(100), Some(2_000), ts + 2),
        ])
        .await
        .unwrap();

    // list_by_decision("rd_run1") returns both calls for run_1.
    let run1_calls = ProviderCallReadModel::list_by_decision(
        &store, &RouteDecisionId::new("rd_run1"), 10,
    ).await.unwrap();
    assert_eq!(run1_calls.len(), 2, "two calls for route decision rd_run1");
    let run1_ids: Vec<_> = run1_calls.iter().map(|c| c.provider_call_id.as_str()).collect();
    assert!(run1_ids.contains(&"call_d1_a"));
    assert!(run1_ids.contains(&"call_d1_b"));

    // list_by_decision("rd_run2") returns only run_2's call.
    let run2_calls = ProviderCallReadModel::list_by_decision(
        &store, &RouteDecisionId::new("rd_run2"), 10,
    ).await.unwrap();
...
async fn list_by_decision_limit_is_respected() {
    ...
    for i in 0u32..5 {
        store.append(&[completed_call(
            &format!("e{i}"), &format!("call_lim_{i}"), "rd_lim", &format!("ra_{i}"),
            "gpt-4o", None, None, ProviderCallStatus::Succeeded,
            Some(100 + i as u64 * 10), Some(256), Some(128), Some(5_000), ts + i as u64,
        )]).await.unwrap();
    }

    let limited = ProviderCallReadModel::list_by_decision(
        &store, &RouteDecisionId::new("rd_lim"), 3,
    ).await.unwrap();
...
async fn run_cost_accumulates_across_calls_in_same_run() {
    ...
    store
        .append(&[
            completed_call("e1", "c1", "rd_1", "ra_1", "gpt-4o",
                Some("run_acc"), None, ProviderCallStatus::Succeeded,
                Some(100), Some(500), Some(250), Some(10_000), ts),
            completed_call("e2", "c2", "rd_2", "ra_2", "gpt-4o",
                Some("run_acc"), None, ProviderCallStatus::Succeeded,
                Some(120), Some(400), Some(200), Some(8_000), ts + 1),
            completed_call("e3", "c3", "rd_3", "ra_3", "gpt-4o",
                Some("run_acc"), None, ProviderCallStatus::Succeeded,
                Some(90), Some(300), Some(150), Some(6_000), ts + 2),
        ])
        .await
        .unwrap();
    ...
    assert_eq!(run_cost.total_cost_micros, 24_000,
        "10_000 + 8_000 + 6_000 = 24_000 µUSD");
    assert_eq!(run_cost.total_tokens_in, 1_200,
        "500 + 400 + 300 = 1_200 input tokens");
    assert_eq!(run_cost.total_tokens_out, 600,
        "250 + 200 + 150 = 600 output tokens");
    assert_eq!(run_cost.provider_calls, 3,
...
async fn session_aggregates_costs_across_multiple_runs() {
    ...
    store
        .append(&[
            completed_call("e1", "ca1", "rda1", "ra1", "gpt-4o",
                Some("run_a"), Some("sess_multi"), ProviderCallStatus::Succeeded,
                Some(100), Some(400), Some(200), Some(12_000), ts),
            completed_call("e2", "ca2", "rda2", "ra2", "gpt-4o",
                Some("run_a"), Some("sess_multi"), ProviderCallStatus::Succeeded,
                Some(110), Some(300), Some(150), Some(9_000), ts + 1),
            completed_call("e3", "cb1", "rdb1", "rb1", "gpt-4o-mini",
                Some("run_b"), Some("sess_multi"), ProviderCallStatus::Succeeded,
                Some(80), Some(200), Some(100), Some(3_000), ts + 2),
        ])
        .await
        .unwrap();
...
async fn cost_micros_accumulates_on_each_call() {
    ...
    for (i, cost) in [5_000u64, 8_000, 3_000, 12_000].iter().enumerate() {
        store.append(&[completed_call(
            &format!("e{i}"), &format!("call_cost_{i}"), &format!("rd_c{i}"), &format!("ra_c{i}"),
            "gpt-4o", Some("run_cost_acc"), None, ProviderCallStatus::Succeeded,
            Some(100), Some(200), Some(100), Some(*cost), ts + i as u64,
        )]).await.unwrap();
    }
    ...
    assert_eq!(record.total_cost_micros, 28_000,
...
async fn free_calls_do_not_add_to_cost() {
    ...
    store
        .append(&[
            completed_call("e1", "free_1", "rd_free", "ra_1", "llama-free",
                Some("run_free"), None, ProviderCallStatus::Succeeded,
                Some(50), Some(100), Some(50), Some(0), ts),
            completed_call("e2", "free_2", "rd_free2", "ra_2", "llama-free",
                Some("run_free"), None, ProviderCallStatus::Succeeded,
                Some(45), Some(80), Some(40), Some(0), ts + 1),
        ])
        .await
        .unwrap();
...
async fn latency_ms_queryable_from_individual_records() {
    ...
    for (i, &lat) in latencies.iter().enumerate() {
        store.append(&[completed_call(
            &format!("e{i}"), &format!("call_lat_{i}"), &format!("rd_lat{i}"), &format!("ra_{i}"),
            "gpt-4o", None, None, ProviderCallStatus::Succeeded,
            Some(lat), Some(200), Some(100), Some(5_000), ts + i as u64,
        )]).await.unwrap();
    }
    ...
    for i in 0..5 {
        let r = ProviderCallReadModel::get(
            &store, &ProviderCallId::new(format!("call_lat_{i}")),
        ).await.unwrap().unwrap();
...
async fn failed_calls_without_latency_have_none() {
    ...
    store.append(&[completed_call(
        "e1", "call_timeout", "rd_to", "ra_to",
        "gpt-4o", None, None, ProviderCallStatus::Failed,
        None, None, None, None, ts,
    )]).await.unwrap();

    let r = ProviderCallReadModel::get(&store, &ProviderCallId::new("call_timeout"))
        .await.unwrap().unwrap();
    assert!(r.latency_ms.is_none(), "failed call with no timing has None latency");
...
async fn provider_model_id_preserved_in_audit_record() {
    ...
    for (i, (call_id, rd_id, model)) in models.iter().enumerate() {
        store.append(&[completed_call(
            &format!("em{i}"), call_id, rd_id, &format!("ra_m{i}"),
            model, None, None, ProviderCallStatus::Succeeded,
            Some(100), Some(200), Some(100), Some(5_000), ts + i as u64,
        )]).await.unwrap();
    }
    ...
    for (call_id, _, expected_model) in &models {
        let r = ProviderCallReadModel::get(&store, &ProviderCallId::new(*call_id))
            .await.unwrap().unwrap();
        assert_eq!(r.provider_model_id.as_str(), *expected_model,
