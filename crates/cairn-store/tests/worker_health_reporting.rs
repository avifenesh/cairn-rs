...
fn project(n: &str) -> ProjectKey {
    ProjectKey::new(format!("tenant_w_{n}"), "ws", "proj")
}
...
async fn worker_registered_with_default_health() {
    ...
    store
        .append(&[register_event("alpha", "1", 1_000)])
        .await
        .unwrap();
    ...
    assert_eq!(
        rec.health.last_heartbeat_ms, 0,
        "no heartbeat yet — timestamp must be 0"
    );
    assert!(
        rec.current_task_id.is_none(),
        "no task assigned at registration"
    );
}
...
async fn heartbeat_updates_health_fields() {
    ...
    store
        .append(&[register_event("beta", "1", 1_000)])
        .await
        .unwrap();

    // First heartbeat at t=5000.
    store
        .append(&[heartbeat_event(
            "beta",
            "1",
            "task_1",
            5_000,
            Some("processing chunk 1"),
        )])
        .await
        .unwrap();
    ...
    assert!(
        after_first.health.is_alive,
        "is_alive must be true after first heartbeat"
    );
    assert_eq!(
        after_first.health.last_heartbeat_ms, 5_000,
        "last_heartbeat_ms must be 5000"
    );
...
async fn current_task_id_assigned_and_cleared() {
    ...
    store
        .append(&[register_event("gamma", "1", 1_000)])
        .await
        .unwrap();

    // Heartbeat with task — current_task_id is set.
    store
        .append(&[heartbeat_event("gamma", "1", "task_x", 5_000, None)])
        .await
        .unwrap();
    ...
    store
        .append(&[terminal_event("gamma", "1", "task_x", 8_000)])
        .await
        .unwrap();
    ...
    assert!(
        after_terminal.health.is_alive,
        "is_alive stays true after terminal report"
    );
...
async fn failed_outcome_clears_current_task_id() {
    let store = Arc::new(InMemoryStore::new());
    store
        .append(&[register_event("delta", "1", 1_000)])
        .await
        .unwrap();
    store
        .append(&[heartbeat_event("delta", "1", "task_y", 3_000, None)])
        .await
        .unwrap();
    ...
    assert!(
        rec.current_task_id.is_none(),
        "Failed outcome must clear current_task_id"
    );
...
async fn stale_detection_workers_without_heartbeat() {
    ...
    store
        .append(&[heartbeat_event("stale_b", "2", "task_b", 10_000, None)])
        .await
        .unwrap();

    let workers = ExternalWorkerReadModel::list_by_tenant(store.as_ref(), &tenant("2"), 10, 0)
        .await
        .unwrap();
    ...
    assert_eq!(
        stale.len(),
        2,
        "stale_a and stale_c are stale (no heartbeat)"
    );
    ...
    for w in &stale {
        ...
        assert_eq!(
            w.health.last_heartbeat_ms, 0,
            "stale worker must have heartbeat=0"
        );
    }
...
async fn list_by_tenant_health_ordering() {
    ...
    let workers = ExternalWorkerReadModel::list_by_tenant(store.as_ref(), &tenant("3"), 10, 0)
        .await
        .unwrap();
    ...
    for window in workers.windows(2) {
        assert!(
            ...
            window[0].registered_at,
            window[1].registered_at
        );
    }
    ...
    let w1 = workers
        .iter()
        .find(|w| w.worker_id == worker("w1"))
        .unwrap();
    let w3 = workers
        .iter()
        .find(|w| w.worker_id == worker("w3"))
        .unwrap();
    ...
    let page1 = ExternalWorkerReadModel::list_by_tenant(store.as_ref(), &tenant("3"), 2, 0)
        .await
        .unwrap();
    let page2 = ExternalWorkerReadModel::list_by_tenant(store.as_ref(), &tenant("3"), 2, 2)
        .await
        .unwrap();
    assert_eq!(page1.len(), 2);
    assert_eq!(page2.len(), 2);
    assert_ne!(
        page1[0].worker_id, page2[0].worker_id,
