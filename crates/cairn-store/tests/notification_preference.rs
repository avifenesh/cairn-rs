...
use cairn_store::{
    projections::NotificationReadModel,
    EventLog, InMemoryStore,
};

// ── helpers ───────────────────────────────────────────────────────────────────

fn tenant(n: &str) -> TenantId { TenantId::new(format!("tenant_notif_{n}")) }
...
fn channel(kind: &str, target: &str) -> NotificationChannel {
    NotificationChannel { kind: kind.to_owned(), target: target.to_owned() }
}
...
async fn preference_set_is_stored_and_readable() {
    ...
    let pref = NotificationReadModel::get_preferences(
        store.as_ref(), &tenant("a"), "operator_alice",
    ).await.unwrap()
...
async fn preference_update_replaces_previous() {
    ...
    store.append(&[pref_event(
        ...
        vec![channel("slack", "#ops"), channel("email", "bob@example.com")],
        2_000,
    )]).await.unwrap();

    let pref = NotificationReadModel::get_preferences(
        store.as_ref(), &tenant("update"), "operator_bob",
    ).await.unwrap().unwrap();

    // Latest preference wins.
    assert_eq!(pref.event_types.len(), 3, "updated event types must replace old ones");
    assert!(pref.event_types.contains(&"cost_alert".to_owned()));
    assert_eq!(pref.channels.len(), 2, "updated channels must replace old channels");
}
...
async fn notification_sent_is_recorded() {
    ...
    store.append(&[sent_event(
        "rec_001", "b", "operator_carol",
        "run_stalled", "email", "carol@example.com",
        true, None, 5_000,
    )]).await.unwrap();

    // (4) Verify the notification record.
    let records = NotificationReadModel::list_sent_notifications(
        store.as_ref(), &tenant("b"), 0,
    ).await.unwrap();
    ...
    assert!(rec.payload.get("event_type").is_some(), "payload must be preserved");
}
...
async fn per_tenant_preference_scoping() {
    ...
    store.append(&[
        pref_event("ta", "op_a1", &["task_failed"], vec![channel("email", "a1@example.com")], 1_000),
        pref_event("ta", "op_a2", &["run_stalled"], vec![channel("slack", "#eng")],           2_000),
    ]).await.unwrap();

    // Tenant B: 1 operator.
    store.append(&[
        pref_event("tb", "op_b1", &["cost_alert"],  vec![channel("email", "b1@example.com")], 3_000),
    ]).await.unwrap();

    // Tenant A sees its own 2 preferences.
    let a_prefs = NotificationReadModel::list_preferences_by_tenant(
        store.as_ref(), &tenant("ta"),
    ).await.unwrap();
    assert_eq!(a_prefs.len(), 2, "tenant A must have 2 preferences");
    assert!(a_prefs.iter().all(|p| p.tenant_id == tenant("ta")),
        "all tenant A preferences must be scoped to tenant A");

    // Tenant B sees only its own 1 preference.
    let b_prefs = NotificationReadModel::list_preferences_by_tenant(
        store.as_ref(), &tenant("tb"),
    ).await.unwrap();
    ...
    let cross = NotificationReadModel::get_preferences(
        store.as_ref(), &tenant("tb"), "op_a1",
    ).await.unwrap();
    assert!(cross.is_none(), "tenant B must not see tenant A's op_a1 preference");
}
...
async fn multiple_channels_coexist() {
    ...
    let pref = NotificationReadModel::get_preferences(
        store.as_ref(), &tenant("multi"), "operator_dave",
    ).await.unwrap().unwrap();
    ...
    assert!(kinds.contains(&"webhook"), "webhook channel must be present");
    ...
    for (kind, target) in [
        ...
        store.append(&[sent_event(
            &format!("rec_multi_{kind}"), "multi", "operator_dave",
            "approval_required", kind, target, true, None, 2_000,
        )]).await.unwrap();
    }

    let records = NotificationReadModel::list_sent_notifications(
        store.as_ref(), &tenant("multi"), 0,
    ).await.unwrap();
...
async fn failed_deliveries_are_separable_from_successes() {
    ...
    store.append(&[
        pref_event("fail", "op_fail", &["run_failed"], vec![channel("webhook", "https://bad.example.com")], 1_000),
        sent_event("rec_ok",   "fail", "op_fail", "run_failed", "email",   "op@example.com",           true,  None,                       2_000),
        sent_event("rec_fail", "fail", "op_fail", "run_failed", "webhook", "https://bad.example.com",   false, Some("connection refused"), 3_000),
    ]).await.unwrap();

    // list_sent_notifications returns both.
    let all = NotificationReadModel::list_sent_notifications(
        store.as_ref(), &tenant("fail"), 0,
    ).await.unwrap();
    assert_eq!(all.len(), 2);

    // list_failed_notifications returns only the failed one.
    let failed = NotificationReadModel::list_failed_notifications(
        store.as_ref(), &tenant("fail"),
    ).await.unwrap();
    assert_eq!(failed.len(), 1, "only 1 failed notification");
    assert_eq!(failed[0].record_id, "rec_fail");
    assert!(!failed[0].delivered);
    assert_eq!(failed[0].delivery_error.as_deref(), Some("connection refused"));

    // since_ms filter: records sent before ts=2500 → only rec_ok (ts=2000).
    let since_2500 = NotificationReadModel::list_sent_notifications(
        store.as_ref(), &tenant("fail"), 2_500,
    ).await.unwrap();
