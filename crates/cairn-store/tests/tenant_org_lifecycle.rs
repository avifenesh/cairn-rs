...
fn tenant_id(n: &str) -> TenantId {
    TenantId::new(format!("tenant_{n}"))
}
fn workspace_id(n: &str) -> WorkspaceId {
    WorkspaceId::new(format!("ws_{n}"))
}
fn project_key(t: &str, w: &str, p: &str) -> ProjectKey {
    ProjectKey::new(
        format!("tenant_{t}"),
        format!("ws_{w}"),
        format!("proj_{p}"),
...
async fn tenant_not_created_returns_none() {
    let store = Arc::new(InMemoryStore::new());
    let missing = TenantReadModel::get(store.as_ref(), &tenant_id("ghost"))
        .await
        .unwrap();
...
async fn workspace_created_stores_record() {
    ...
    store
        .append(&[tenant_event("beta"), workspace_event("beta", "eng")])
        .await
        .unwrap();
    ...
    assert_eq!(
        ws.tenant_id,
        tenant_id("beta"),
        "workspace must carry its tenant"
    );
...
async fn full_hierarchy_tenant_workspace_project() {
    ...
    let proj =
        ProjectReadModel::get_project(store.as_ref(), &project_key("corp", "platform", "api"))
            .await
            .unwrap()
            .unwrap();
    ...
    assert_eq!(
        proj.tenant_id,
        tenant_id("corp"),
        "project must carry its tenant through workspace"
    );
...
async fn list_by_tenant_returns_all_workspaces_for_tenant() {
    ...
    let mega_ws = WorkspaceReadModel::list_by_tenant(store.as_ref(), &tenant_id("megacorp"), 10, 0)
        .await
        .unwrap();
    assert_eq!(mega_ws.len(), 3, "megacorp must list exactly 3 workspaces");
    assert!(
        mega_ws.iter().all(|w| w.tenant_id == tenant_id("megacorp")),
        "all listed workspaces must belong to megacorp"
    );

    let ws_names: Vec<&str> = mega_ws.iter().map(|w| w.workspace_id.as_str()).collect();
    assert!(
        ws_names.contains(&"ws_backend"),
        "ws_backend must be listed"
    );
    assert!(
        ws_names.contains(&"ws_frontend"),
        "ws_frontend must be listed"
    );
    assert!(ws_names.contains(&"ws_data"), "ws_data must be listed");

    // rival must not see megacorp workspaces.
    let rival_ws = WorkspaceReadModel::list_by_tenant(store.as_ref(), &tenant_id("rival"), 10, 0)
        .await
        .unwrap();
    ...
    let first_page =
        WorkspaceReadModel::list_by_tenant(store.as_ref(), &tenant_id("megacorp"), 2, 0)
            .await
            .unwrap();
    let second_page =
        WorkspaceReadModel::list_by_tenant(store.as_ref(), &tenant_id("megacorp"), 2, 2)
            .await
            .unwrap();
...
async fn workspace_scoping_is_cascade_boundary() {
    ...
    assert_eq!(
        alpha_projects.len(),
        2,
        "workspace alpha must contain 2 projects"
    );
    let alpha_ids: Vec<_> = alpha_projects
        .iter()
        .map(|p| p.project_id.as_str())
        .collect();
    assert!(
        alpha_ids.contains(&"proj_auth"),
        "proj_auth must be in alpha"
    );
    assert!(
        alpha_ids.contains(&"proj_billing"),
        "proj_billing must be in alpha"
    );
    ...
    assert_eq!(
        beta_projects.len(),
        1,
        "workspace beta must contain only 1 project"
    );
    assert_eq!(beta_projects[0].project_id.as_str(), "proj_analytics");
    assert!(
        !beta_projects
            .iter()
            .any(|p| alpha_ids.contains(&p.project_id.as_str())),
...
async fn cross_tenant_org_isolation() {
    ...
    let beta_workspaces =
        WorkspaceReadModel::list_by_tenant(store.as_ref(), &tenant_id("beta_corp"), 10, 0)
            .await
            .unwrap();
    assert_eq!(beta_workspaces.len(), 1);
    assert_eq!(
        beta_workspaces[0].workspace_id.as_str(),
        "ws_eng_b",
    ...
    assert_ne!(
        beta_projects[0].project_id.as_str(),
        "proj_alpha_proj",
