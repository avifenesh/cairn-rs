...
fn content_hash(content: &str) -> String {
    ...
    format!("sha256:{:08x}{:08x}{:08x}", content.len(), first as u32, last as u32)
}
...
async fn create_prompt_asset_is_stored() {
    ...
    let asset = PromptAssetReadModel::get(&store, &asset_id)
        .await.unwrap().expect("asset must exist");
...
async fn create_version_v1_with_hello_world() {
    ...
    let v1 = PromptVersionReadModel::get(&store, &v1_id)
        .await.unwrap().expect("v1 must exist");
...
async fn create_version_v2_with_different_content() {
    ...
    let v2 = PromptVersionReadModel::get(&store, &v2_id)
        .await.unwrap().expect("v2 must exist");
    assert_eq!(v2.content_hash, hash_v2);
    assert_ne!(hash_v1, hash_v2, "different content must produce different hashes");
}
...
async fn both_versions_are_retrievable() {
    ...
    assert_ne!(found_v1.unwrap().content_hash, found_v2.unwrap().content_hash);
}
...
async fn version_number_auto_increments_per_asset() {
    ...
    for i in 1u32..=5 {
        ...
        let v = PromptVersionReadModel::get(&store, &PromptVersionId::new(format!("ver_inc_{i}")))
            .await.unwrap().unwrap();
        assert_eq!(v.version_number, i, "version {i} must have version_number={i}");
    }
    ...
    for (i, v) in all.iter().enumerate() {
        assert_eq!(v.version_number, (i + 1) as u32,
            "position {i} must have version_number {}", i + 1);
    }
}
...
async fn content_hash_differs_between_versions() {
    ...
    assert_eq!(hashes.len(), 3, "every version must have a unique content_hash");
...
async fn versions_are_isolated_by_workspace() {
    ...
    assert!(alpha_versions.iter().all(|v| v.project.workspace_id.as_str() == "ws_alpha"),
    ...
    let alpha_ids: std::collections::HashSet<_> =
        alpha_versions.iter().map(|v| v.prompt_version_id.as_str()).collect();
    let beta_ids: std::collections::HashSet<_> =
        beta_versions.iter().map(|v| v.prompt_version_id.as_str()).collect();
...
async fn version_numbers_are_per_asset_not_global() {
    ...
    // asset_x: vx1=1, vx2=2.
    let vx1 = PromptVersionReadModel::get(&store, &PromptVersionId::new("vx1")).await.unwrap().unwrap();
    let vx2 = PromptVersionReadModel::get(&store, &PromptVersionId::new("vx2")).await.unwrap().unwrap();
    ...
    let vy1 = PromptVersionReadModel::get(&store, &PromptVersionId::new("vy1")).await.unwrap().unwrap();
    let vy2 = PromptVersionReadModel::get(&store, &PromptVersionId::new("vy2")).await.unwrap().unwrap();
    assert_eq!(vy1.version_number, 1, "asset_y version counter is independent of asset_x");
...
async fn list_by_project_returns_only_project_assets() {
    ...
    assert!(x_assets.iter().all(|a| a.project.workspace_id.as_str() == "ws_x"));
