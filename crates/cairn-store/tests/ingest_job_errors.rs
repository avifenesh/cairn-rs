...
use cairn_domain::{
    EventEnvelope, EventId, EventSource, IngestJobId, ProjectKey, RuntimeEvent, SourceId,
};
...
fn project_a() -> ProjectKey {
    ProjectKey::new("tenant_ingest", "ws_ingest", "proj_a")
}
fn project_b() -> ProjectKey {
    ProjectKey::new("tenant_ingest", "ws_ingest", "proj_b")
}

fn job_id(n: &str) -> IngestJobId {
    IngestJobId::new(format!("job_{n}"))
}
...
async fn failed_ingest_job_state_persisted() {
    ...
    store
        .append(&[start_event("fail1", project_a(), 20, 1_000)])
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        "job must start in Processing"
    );
    assert_eq!(
        started.document_count, 20,
        "document_count must be set from start event"
    );
    assert!(
        started.error_message.is_none(),
        "no error yet while Processing"
    );

    // (2) Complete with failure.
    store
        .append(&[complete_failure(
            "fail1",
            project_a(),
    ...
    assert_eq!(
        ...
        "state must be Failed after success=false"
    );
    assert!(
    ...
    assert_eq!(
        failed.updated_at, 2_000,
        "updated_at must reflect completion timestamp"
    );
}
...
async fn partial_completion_preserves_document_count() {
    ...
    store
        .append(&[start_event("partial", project_a(), 500, 1_000)])
        .await
        .unwrap();
...
async fn list_by_project_returns_failed_and_succeeded_jobs() {
    ...
    store
        .append(&[
            ...
            complete_failure(
                "f1",
                project_a(),
                "parse error: invalid UTF-8 in document 12",
                8_000,
            ),
            complete_failure(
                "f2",
                project_a(),
                "timeout waiting for embeddings API",
                9_000,
            ),
    ...
    let completed: Vec<_> = jobs
        .iter()
        .filter(|j| j.state == IngestJobState::Completed)
        .collect();
    let failed: Vec<_> = jobs
        .iter()
        .filter(|j| j.state == IngestJobState::Failed)
        .collect();
    let processing: Vec<_> = jobs
        .iter()
        .filter(|j| j.state == IngestJobState::Processing)
        .collect();
    ...
    assert!(
        completed.iter().all(|j| j.error_message.is_none()),
...
async fn error_message_round_trips_without_loss() {
    ...
    let test_cases: &[(&str, &str)] = &[
        (
            "ascii",
            "connection refused: TCP connect to 10.0.0.1:9200 failed",
        ),
        (
            "unicode",
            "解析エラー: 無効なUTF-8シーケンス at byte offset 1024",
        ),
        (
            "json_err",
            r#"{"code": "RATE_LIMIT", "retry_after": 60, "limit": "100/min"}"#,
        ),
        (
            "multiline",
...
async fn ingest_jobs_are_project_scoped() {
    ...
    assert!(
        a_jobs.iter().all(|j| j.project == project_a()),
...
async fn entity_scoped_read_returns_ingest_job_events() {
    ...
    assert_eq!(
        job_events.len(),
        2,
        "IngestJobStarted + IngestJobCompleted = 2 events"
    );
    assert!(
        matches!(&job_events[0].envelope.payload, RuntimeEvent::IngestJobStarted(s) if s.job_id == job_id("entity1"))
    );
    assert!(
        matches!(&job_events[1].envelope.payload, RuntimeEvent::IngestJobCompleted(c) if !c.success)
    );
}
