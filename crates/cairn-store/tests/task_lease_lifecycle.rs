...
fn run_id() -> RunId   { RunId::new("run_lease_1") }
fn task_id(n: &str) -> TaskId { TaskId::new(format!("task_{n}")) }
...
async fn seed_task(store: &Arc<InMemoryStore>, tid: &str) {
    ...
    store.append(&[
        ev(&format!("evt_sess_{tid}"), RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: sess.clone(),
        })),
        ev(&format!("evt_run_{tid}"), RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: sess, run_id: run_id(),
            parent_run_id: None, prompt_release_id: None, agent_role_id: None,
        })),
        ev(&format!("evt_task_{tid}"), RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id(tid),
...
async fn task_lease_claimed_updates_record() {
    ...
    let queued = TaskReadModel::get(store.as_ref(), &task_id("alpha")).await.unwrap().unwrap();
    ...
    let leased = TaskReadModel::get(store.as_ref(), &task_id("alpha")).await.unwrap().unwrap();

    // State transitions to Leased.
    assert_eq!(leased.state, TaskState::Leased, "task must be Leased after claim");

    // Lease metadata is set.
    assert_eq!(leased.lease_owner.as_deref(), Some("worker_1"), "lease_owner must be worker_1");
    assert_eq!(leased.lease_expires_at, Some(100_000), "lease_expires_at must be set");

    // Version must have incremented (claimed = 2 events).
    assert!(leased.version > queued.version, "version must increment after lease events");
}
...
async fn task_lease_heartbeat_extends_expiry() {
    ...
    let after_claim = TaskReadModel::get(store.as_ref(), &task_id("beta")).await.unwrap().unwrap();
    ...
    let after_hb = TaskReadModel::get(store.as_ref(), &task_id("beta")).await.unwrap().unwrap();

    assert_eq!(
        after_hb.lease_expires_at, Some(90_000),
        "lease_expires_at must be extended by heartbeat"
    );
    assert_eq!(
        after_hb.lease_owner.as_deref(), Some("worker_2"),
...
async fn expired_leases_detected_at_read_time() {
    ...
    let expired = TaskReadModel::list_expired_leases(store.as_ref(), 5_000, 100)
        .await.unwrap();
    assert_eq!(expired.len(), 1, "task with past TTL must appear in expired list");
...
async fn expired_lease_allows_reclaim_after_recovery() {
    ...
    let expired = TaskReadModel::list_expired_leases(store.as_ref(), 9_999, 100)
        .await.unwrap();
    assert_eq!(expired.len(), 1, "task must appear in expired list before recovery");

    // Recovery sweep: re-queue the task (clear lease + set state back to Queued).
    store.append(&[
        ev("evt_requeue_reclaim", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(),
            task_id: task_id("reclaim"),
            transition: StateTransition { from: Some(TaskState::Leased), to: TaskState::Queued },
            failure_class: None, pause_reason: None, resume_trigger: None,
        })),
    ]).await.unwrap();

    // After re-queue the task is Queued again with no lease.
    let requeued = TaskReadModel::get(store.as_ref(), &task_id("reclaim")).await.unwrap().unwrap();
    assert_eq!(requeued.state, TaskState::Queued, "task must be Queued after recovery");

    // New worker claims the lease.
    claim_lease(&store, "reclaim", "worker_new", 2, 50_000).await;

    let reclaimed = TaskReadModel::get(store.as_ref(), &task_id("reclaim")).await.unwrap().unwrap();
    assert_eq!(reclaimed.state, TaskState::Leased);
    assert_eq!(reclaimed.lease_owner.as_deref(), Some("worker_new"),
...
async fn list_by_parent_run_returns_tasks_with_lease_state() {
    ...
    store.append(&[
        ev("sess_lr", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: SessionId::new("sess_lr"),
        })),
        ev("run_lr", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new("sess_lr"), run_id: run_id(),
            parent_run_id: None, prompt_release_id: None, agent_role_id: None,
        })),
        // Three tasks under run_lease_1.
        ev("evt_task_x", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id("x"),
            parent_run_id: Some(run_id()), parent_task_id: None, prompt_release_id: None,
        })),
        ev("evt_task_y", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id("y"),
            parent_run_id: Some(run_id()), parent_task_id: None, prompt_release_id: None,
        })),
        ev("evt_task_z", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id("z"),
            parent_run_id: Some(run_id()), parent_task_id: None, prompt_release_id: None,
        })),
    ]).await.unwrap();
    ...
    store.append(&[
        ev("evt_claim_x", RuntimeEvent::TaskLeaseClaimed(TaskLeaseClaimed {
            project: project(), task_id: task_id("x"),
            lease_owner: "worker_x".to_owned(), lease_token: 10, lease_expires_at_ms: 80_000,
        })),
        ev("evt_state_x", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(), task_id: task_id("x"),
    ...
    let task_x = tasks.iter().find(|t| t.task_id == task_id("x"))
        .expect("task_x must be in the list");
    assert_eq!(task_x.state, TaskState::Leased, "task_x must be Leased");
    assert_eq!(task_x.lease_owner.as_deref(), Some("worker_x"), "task_x lease owner correct");
    ...
    for name in ["y", "z"] {
        ...
        assert!(t.lease_owner.is_none(), "task_{name} must have no lease owner");
    }
}
...
async fn concurrent_leases_on_different_tasks() {
    ...
    store.append(&[
        ev("sess_c", RuntimeEvent::SessionCreated(SessionCreated {
            project: project(), session_id: SessionId::new("sess_c"),
        })),
        ev("run_c", RuntimeEvent::RunCreated(RunCreated {
            project: project(), session_id: SessionId::new("sess_c"), run_id: run_id(),
            parent_run_id: None, prompt_release_id: None, agent_role_id: None,
        })),
        ev("task_p", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id("p"),
            parent_run_id: Some(run_id()), parent_task_id: None, prompt_release_id: None,
        })),
        ev("task_q", RuntimeEvent::TaskCreated(TaskCreated {
            project: project(), task_id: task_id("q"),
            parent_run_id: Some(run_id()), parent_task_id: None, prompt_release_id: None,
        })),
    ]).await.unwrap();
    ...
    store.append(&[
        ev("claim_p", RuntimeEvent::TaskLeaseClaimed(TaskLeaseClaimed {
            project: project(), task_id: task_id("p"),
            lease_owner: "worker_A".to_owned(), lease_token: 1, lease_expires_at_ms: 60_000,
        })),
        ev("state_p", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(), task_id: task_id("p"),
            transition: StateTransition { from: Some(TaskState::Queued), to: TaskState::Leased },
            failure_class: None, pause_reason: None, resume_trigger: None,
        })),
        ev("claim_q", RuntimeEvent::TaskLeaseClaimed(TaskLeaseClaimed {
            project: project(), task_id: task_id("q"),
            lease_owner: "worker_B".to_owned(), lease_token: 2, lease_expires_at_ms: 60_000,
        })),
        ev("state_q", RuntimeEvent::TaskStateChanged(TaskStateChanged {
            project: project(), task_id: task_id("q"),
            transition: StateTransition { from: Some(TaskState::Queued), to: TaskState::Leased },
            failure_class: None, pause_reason: None, resume_trigger: None,
        })),
    ...
    let task_p = TaskReadModel::get(store.as_ref(), &task_id("p")).await.unwrap().unwrap();
    let task_q = TaskReadModel::get(store.as_ref(), &task_id("q")).await.unwrap().unwrap();

    assert_eq!(task_p.lease_owner.as_deref(), Some("worker_A"));
    assert_eq!(task_q.lease_owner.as_deref(), Some("worker_B"));
    assert_ne!(task_p.lease_owner, task_q.lease_owner,
