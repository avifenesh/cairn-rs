...
use cairn_domain::{
    EventEnvelope, EventId, EventSource, TenantId, WorkspaceId, RuntimeEvent,
    events::{ResourceShared, ResourceShareRevoked},
    tenancy::OwnershipKey,
};
use cairn_store::{
    projections::ResourceSharingReadModel,
...
async fn resource_shared_appears_in_read_model() {
    ...
    share_resource(
        &store, "e1", "share_1", "tenant_a",
        "ws_src", "ws_tgt", "prompt_asset", "asset_42",
        vec!["read"], 1_000,
    ).await;
...
async fn get_share_returns_none_for_unknown_id() {
    let store = InMemoryStore::new();
    let result = ResourceSharingReadModel::get_share(&store, "ghost_share").await.unwrap();
...
async fn resource_shared_with_multiple_permissions_preserves_all() {
    ...
    share_resource(
        &store, "e1", "share_perms", "tenant_p",
        "ws_from", "ws_to", "corpus", "corpus_99",
        vec!["read", "embed", "export"], 2_000,
    ).await;
...
async fn revoked_share_removed_from_read_model() {
    ...
    share_resource(
        &store, "e1", "share_rev", "tenant_r",
        "ws_a", "ws_b", "source", "src_1", vec!["read"], 1_000,
    ).await;

    // Confirm the grant exists before revocation.
    let before = ResourceSharingReadModel::get_share(&store, "share_rev").await.unwrap();
    ...
    let after = ResourceSharingReadModel::get_share(&store, "share_rev").await.unwrap();
    assert!(after.is_none(), "share must be absent after ResourceShareRevoked");
}
...
async fn revocation_does_not_affect_other_shares() {
    ...
    share_resource(&store, "e1", "share_keep", "tenant_k", "ws_1", "ws_2", "prompt_asset", "pa_1", vec![], 1_000).await;
    share_resource(&store, "e2", "share_gone", "tenant_k", "ws_1", "ws_2", "prompt_asset", "pa_2", vec![], 1_000).await;

    revoke_share(&store, "e3", "share_gone", "tenant_k", 2_000).await;

    let kept = ResourceSharingReadModel::get_share(&store, "share_keep").await.unwrap();
    let gone = ResourceSharingReadModel::get_share(&store, "share_gone").await.unwrap();
...
async fn list_shares_for_workspace_returns_only_target_workspace_shares() {
    ...
    share_resource(&store, "e1", "sh_to_ws_a", tenant, "ws_src", "ws_a", "prompt_asset", "pa_1", vec![], 1_000).await;
    share_resource(&store, "e2", "sh_to_ws_a2", tenant, "ws_src", "ws_a", "corpus", "corp_1", vec![], 1_000).await;
    share_resource(&store, "e3", "sh_to_ws_b",  tenant, "ws_src", "ws_b", "prompt_asset", "pa_2", vec![], 1_000).await;

    let ws_a_shares = ResourceSharingReadModel::list_shares_for_workspace(
        &store, &TenantId::new(tenant), &WorkspaceId::new("ws_a"),
    ).await.unwrap();

    let ws_b_shares = ResourceSharingReadModel::list_shares_for_workspace(
        &store, &TenantId::new(tenant), &WorkspaceId::new("ws_b"),
    ).await.unwrap();
...
async fn list_shares_for_workspace_returns_empty_for_unknown_workspace() {
    let store = InMemoryStore::new();
    share_resource(&store, "e1", "sh_x", "tenant_empty", "ws_src", "ws_known", "source", "s1", vec![], 1_000).await;

    let result = ResourceSharingReadModel::list_shares_for_workspace(
        &store, &TenantId::new("tenant_empty"), &WorkspaceId::new("ws_unknown"),
    ).await.unwrap();
...
async fn shares_are_scoped_by_tenant() {
    ...
    share_resource(&store, "e1", "sh_ta", "tenant_ta", "ws_1", "ws_2", "prompt_asset", "pa", vec![], 1_000).await;
    share_resource(&store, "e2", "sh_tb", "tenant_tb", "ws_1", "ws_2", "prompt_asset", "pa", vec![], 1_000).await;

    let ta_shares = ResourceSharingReadModel::list_shares_for_workspace(
        &store, &TenantId::new("tenant_ta"), &WorkspaceId::new("ws_2"),
    ).await.unwrap();
    let tb_shares = ResourceSharingReadModel::list_shares_for_workspace(
        &store, &TenantId::new("tenant_tb"), &WorkspaceId::new("ws_2"),
    ).await.unwrap();
...
async fn get_share_for_resource_finds_exact_grant() {
    ...
    share_resource(&store, "e1", "sh_exact", tenant, "ws_a", "ws_b", "corpus", "corp_xyz", vec!["read"], 1_000).await;
    // Red herring: same resource_type + resource_id but different target workspace.
    share_resource(&store, "e2", "sh_other_ws", tenant, "ws_a", "ws_c", "corpus", "corp_xyz", vec![], 1_000).await;
...
async fn get_share_for_resource_returns_none_after_revocation() {
    ...
    share_resource(&store, "e1", "sh_rev_res", tenant, "ws_src", "ws_tgt", "prompt_asset", "pa_r", vec![], 1_000).await;
    ...
    assert!(result.is_none(), "revoked share must not be returned by get_share_for_resource");
}
...
async fn revoking_nonexistent_share_is_a_no_op() {
    ...
    let result = ResourceSharingReadModel::get_share(&store, "never_existed").await.unwrap();
    assert!(result.is_none(), "revoking nonexistent share must not create a phantom record");
}
...
async fn double_revocation_is_safe() {
    ...
    share_resource(&store, "e1", "share_double", "tenant_d", "ws_a", "ws_b", "source", "s1", vec![], 1_000).await;
    ...
    let result = ResourceSharingReadModel::get_share(&store, "share_double").await.unwrap();
    assert!(result.is_none());
}
...
async fn share_and_revoke_events_are_in_log() {
    ...
    share_resource(&store, "e1", "sh_log", "tenant_log", "ws_1", "ws_2", "corpus", "c1", vec![], 1_000).await;
    revoke_share(&store, "e2", "sh_log", "tenant_log", 2_000).await;

    let all = store.read_stream(None, 100).await.unwrap();
    assert_eq!(all.len(), 2);

    assert!(matches!(&all[0].envelope.payload, RuntimeEvent::ResourceShared(e)
        if e.share_id == "sh_log" && e.resource_id == "c1"));
    assert!(matches!(&all[1].envelope.payload, RuntimeEvent::ResourceShareRevoked(e)
...
async fn multiple_shares_coexist_independently() {
    ...
    for i in 0..5u32 {
        let result = ResourceSharingReadModel::get_share(&store, &format!("sh_{i}")).await.unwrap();
        if i == 2 {
