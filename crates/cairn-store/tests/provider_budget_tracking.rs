...
use cairn_store::{projections::ProviderBudgetReadModel, EventLog, InMemoryStore};
...
async fn budget_set_stores_record_with_zero_spend() {
    ...
    assert_eq!(
        ...
        "new budget starts at zero spend"
    );
}
...
async fn daily_and_monthly_budgets_are_independent() {
    ...
    assert_eq!(
        daily.alert_threshold_percent, 80,
        "default alert threshold when None"
    );
...
async fn alert_triggered_updates_current_spend() {
    ...
    assert_eq!(
        budget.current_spend_micros, alert_spend,
...
async fn spend_accumulates_through_alert_progression() {
    ...
    for (spend, evt_id) in steps {
        ...
        assert_eq!(
            budget.current_spend_micros, *spend,
            "after alert at {}µ: current_spend must be {}µ",
            spend, spend
        );
    }
    ...
    assert_eq!(
        final_budget.current_spend_micros,
        limit + 200_000,
...
async fn budgets_scoped_by_tenant() {
    ...
    .unwrap();
    assert_eq!(
        x.current_spend_micros, 4_000_000,
        "tenant_x spend updated by alert"
    );
    ...
    .unwrap();
    assert_eq!(
        y.current_spend_micros, 0,
        "tenant_y spend unaffected by tenant_x alert"
    );
...
async fn list_by_tenant_returns_all_periods() {
    ...
    let daily = budgets
        .iter()
        .find(|b| b.period == ProviderBudgetPeriod::Daily)
        .unwrap();
    let monthly = budgets
        .iter()
        .find(|b| b.period == ProviderBudgetPeriod::Monthly)
        .unwrap();
    assert_eq!(daily.limit_micros, 1_000_000);
...
async fn budget_set_overwrites_existing_record() {
    ...
    assert_eq!(
        after.limit_micros, 10_000_000,
        "limit updated by new ProviderBudgetSet"
    );
    assert_eq!(
        after.current_spend_micros, 0,
        "spend reset to 0 on new budget period"
    );
