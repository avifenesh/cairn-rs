...
async fn connection_registered_and_readable() {
    ...
    store
        .append(&[register_event("openai", "openai")])
        .await
        .unwrap();
    ...
    let connections =
        ProviderConnectionReadModel::list_by_tenant(store.as_ref(), &tenant_id(), 10, 0)
            .await
            .unwrap();
...
async fn health_checked_healthy_updates_record() {
    ...
    store
        .append(&[register_event("anthropic", "anthropic")])
        .await
        .unwrap();

    // Initial health check: healthy, 85ms latency.
    store
        .append(&[health_check_event(
            "anthropic",
    ...
    assert!(
        health.healthy,
        "health record must be healthy after Healthy status check"
    );
    assert_eq!(health.status, ProviderHealthStatus::Healthy);
    assert_eq!(
        health.last_checked_ms, 2_000_000,
        "last_checked_ms must match checked_at_ms"
    );
    assert_eq!(
        health.consecutive_failures, 0,
        "no consecutive failures after healthy check"
    );
    assert!(
        health.error_message.is_none(),
        "no error message for healthy connection"
    );
}
...
async fn full_lifecycle_register_healthy_degraded_recovered() {
    ...
    store
        .append(&[register_event("bedrock", "bedrock")])
        .await
        .unwrap();
    ...
    let health_degraded = ProviderHealthReadModel::get(store.as_ref(), &conn_id("bedrock"))
        ...
        .unwrap();
    assert!(
        !health_degraded.healthy,
        "health must be false after ProviderMarkedDegraded"
    );
    assert_eq!(
    ...
    store
        .append(&[recover_event("bedrock", 5_000_000)])
        .await
        .unwrap();
    ...
    assert_eq!(
        health_recovered.consecutive_failures, 0,
...
async fn consecutive_failures_tracked_and_reset() {
    ...
    store
        .append(&[register_event("openrouter", "openrouter")])
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        "consecutive_failures must reset to 0 after healthy check"
    );
    assert!(h.healthy);
}
...
async fn multiple_connections_tracked_independently() {
    ...
    assert!(
        hc.is_none(),
        "conn_c has no health check yet — record must be None"
    );

    // list_by_tenant returns all registered connections.
    let conns = ProviderConnectionReadModel::list_by_tenant(store.as_ref(), &tenant_id(), 10, 0)
        .await
        .unwrap();
    assert_eq!(
        conns.len(),
        3,
        "all 3 connections must be listed for tenant"
    );

    // list health records by tenant returns only those with health records.
    let health_records =
        ProviderHealthReadModel::list_by_tenant(store.as_ref(), &tenant_id(), 10, 0)
            .await
            .unwrap();
    assert_eq!(
        health_records.len(),
...
async fn degrade_without_prior_health_check_creates_record() {
    ...
    store
        .append(&[register_event("cold_conn", "openai")])
        .await
        .unwrap();
