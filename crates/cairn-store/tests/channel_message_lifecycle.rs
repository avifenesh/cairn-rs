...
use cairn_domain::{
    ChannelId, EventEnvelope, EventId, EventSource, ProjectId, ProjectKey,
    RuntimeEvent, TenantId, WorkspaceId,
    events::{ChannelCreated, ChannelMessageConsumed, ChannelMessageSent},
    tenancy::OwnershipKey,
};
use cairn_store::{
    projections::ChannelReadModel,
...
async fn sent_message_fields_are_preserved() {
    ...
    send_message(&store, "e2", ch, &proj, "msg_f1", "agent_x", "Important payload", 5_000).await;
    ...
    assert!(m.consumed_by.is_none(), "newly sent message must not be consumed");
...
async fn consumed_message_has_consumed_by_and_consumed_at_set() {
    ...
    send_message(&store, "e2", ch, &proj, "msg_consume", "sender_a", "Consume me", 2_000).await;
    ...
    assert_eq!(m.consumed_by.as_deref(), Some("consumer_b"),
        "consumed_by must be set after ChannelMessageConsumed");
    assert_eq!(m.consumed_at_ms, Some(3_000),
...
async fn consume_unknown_message_is_a_no_op() {
    ...
    let messages = ChannelReadModel::list_messages(&store, &ChannelId::new(ch), 10)
        .await.unwrap();
    assert!(messages.is_empty(), "no messages should exist for ghost consume");
}
...
async fn unconsumed_messages_remain_after_first_is_consumed() {
    ...
    for i in 1..=3u32 {
        send_message(
            &store, &format!("e_s_{i}"), ch, &proj,
            &format!("msg_{i}"), "sender", &format!("body {i}"), 1_000 + i as u64,
        ).await;
    }
    ...
    assert_eq!(messages.len(), 3, "all 3 messages (consumed + unconsumed) must be returned");

    let consumed: Vec<_> = messages.iter().filter(|m| m.consumed_by.is_some()).collect();
    let unconsumed: Vec<_> = messages.iter().filter(|m| m.consumed_by.is_none()).collect();
...
async fn consuming_all_messages_marks_all_consumed() {
    ...
    for i in 1..=3u32 {
        send_message(
            &store, &format!("s{i}"), ch, &proj,
            &format!("m{i}"), "s", &format!("b{i}"), i as u64,
        ).await;
        consume_message(
            &store, &format!("c{i}"), ch, &proj,
            &format!("m{i}"), "consumer", 100 + i as u64,
        ).await;
    }
    ...
    assert!(messages.iter().all(|m| m.consumed_by.is_some()),
...
async fn channel_capacity_is_stored_on_record() {
    ...
    assert_eq!(rec.capacity, 5, "capacity must match the ChannelCreated event value");
}
...
async fn channels_with_different_capacities_are_stored_independently() {
    ...
    create_channel(&store, "e3", "chan_unbounded", &proj, "Infinite", u32::MAX, 1_000).await;

    let small  = ChannelReadModel::get_channel(&store, &ChannelId::new("chan_small")).await.unwrap().unwrap();
    let large  = ChannelReadModel::get_channel(&store, &ChannelId::new("chan_large")).await.unwrap().unwrap();
    let unbounded = ChannelReadModel::get_channel(&store, &ChannelId::new("chan_unbounded")).await.unwrap().unwrap();
...
async fn messages_are_scoped_to_their_channel() {
    ...
    send_message(&store, "e3", "chan_iso_a", &proj, "msg_a1", "sender", "For A", 2_000).await;
    send_message(&store, "e4", "chan_iso_a", &proj, "msg_a2", "sender", "Also A", 3_000).await;
    send_message(&store, "e5", "chan_iso_b", &proj, "msg_b1", "sender", "For B", 2_000).await;
...
async fn consuming_message_in_one_channel_does_not_affect_other() {
    ...
    send_message(&store, "e3", "chan_x1", &proj, "shared_msg_id", "s", "body", 2_000).await;
    send_message(&store, "e4", "chan_x2", &proj, "shared_msg_id", "s", "body", 2_000).await;

    // Consume in chan_x1 only.
    consume_message(&store, "e5", "chan_x1", &proj, "shared_msg_id", "consumer", 3_000).await;

    let x1_msgs = ChannelReadModel::list_messages(&store, &ChannelId::new("chan_x1"), 10).await.unwrap();
    let x2_msgs = ChannelReadModel::list_messages(&store, &ChannelId::new("chan_x2"), 10).await.unwrap();

    assert!(x1_msgs[0].consumed_by.is_some(), "chan_x1 message must be consumed");
    assert!(x2_msgs[0].consumed_by.is_none(), "chan_x2 message must remain unconsumed");
}
...
async fn list_channels_scoped_to_project() {
    ...
    let pa_channels = ChannelReadModel::list_channels(&store, &proj_a, 100, 0).await.unwrap();
    let pb_channels = ChannelReadModel::list_channels(&store, &proj_b, 100, 0).await.unwrap();
...
async fn all_lifecycle_events_appear_in_log() {
    ...
    assert!(matches!(&all[0].envelope.payload, RuntimeEvent::ChannelCreated(e)
        if e.channel_id.as_str() == ch));
    assert!(matches!(&all[1].envelope.payload, RuntimeEvent::ChannelMessageSent(e)
        if e.message_id == "msg_log"));
    assert!(matches!(&all[2].envelope.payload, RuntimeEvent::ChannelMessageConsumed(e)
