...
async fn license_activated_appears_in_read_model() {
    ...
    activate_license(&store, "e1", "tenant_lic", "lic_001",
        ProductTier::TeamSelfHosted, 1_000, None).await;
...
async fn license_stores_valid_from_as_issued_at() {
    let store = InMemoryStore::new();
    activate_license(&store, "e1", "tenant_ts", "lic_ts",
        ProductTier::TeamSelfHosted, 7_000, None).await;
...
async fn license_stores_valid_until_as_expires_at_when_set() {
    let store = InMemoryStore::new();
    activate_license(&store, "e1", "tenant_exp", "lic_exp",
        ProductTier::EnterpriseSelfHosted, 1_000, Some(99_999)).await;
    ...
    assert_eq!(rec.expires_at, Some(99_999), "expires_at must be valid_until_ms");
}

#[tokio::test]
async fn license_with_no_expiry_has_expires_at_none() {
    let store = InMemoryStore::new();
    activate_license(&store, "e1", "tenant_noexp", "lic_noexp",
        ProductTier::LocalEval, 5_000, None).await;

    let rec = LicenseReadModel::get_active(&store, &TenantId::new("tenant_noexp"))
        .await.unwrap().unwrap();

    assert!(rec.expires_at.is_none(), "perpetual license must have expires_at = None");
}
...
async fn license_reactivation_overwrites_previous_record() {
    ...
    activate_license(&store, "e1", "tenant_reup", "old_lic",
        ProductTier::LocalEval, 1_000, Some(5_000)).await;

    activate_license(&store, "e2", "tenant_reup", "new_lic",
        ProductTier::EnterpriseSelfHosted, 6_000, None).await;

    let rec = LicenseReadModel::get_active(&store, &TenantId::new("tenant_reup"))
        .await.unwrap().unwrap();

    assert_eq!(rec.license_key.as_deref(), Some("new_lic"),
...
async fn entitlement_override_appears_in_list_overrides() {
    ...
    activate_license(&store, "e1", "tenant_ov", "lic_ov",
        ProductTier::TeamSelfHosted, 1_000, None).await;
    set_override(&store, "e2", "tenant_ov", "eval_matrices", true, None, 2_000).await;
...
async fn list_overrides_returns_empty_for_tenant_with_no_overrides() {
    let store = InMemoryStore::new();
    activate_license(&store, "e1", "tenant_clean", "lic_c",
        ProductTier::LocalEval, 1_000, None).await;
...
async fn override_carries_correct_tenant_id() {
    let store = InMemoryStore::new();
    set_override(&store, "e1", "tenant_link", "multi_provider", true,
        Some("pilot programme"), 3_000).await;

    let overrides = LicenseReadModel::list_overrides(&store, &TenantId::new("tenant_link"))
        .await.unwrap();

    assert_eq!(overrides.len(), 1);
    assert_eq!(overrides[0].tenant_id.as_str(), "tenant_link",
...
async fn disabling_override_is_stored_with_allowed_false() {
    let store = InMemoryStore::new();
    set_override(&store, "e1", "tenant_deny", "eval_matrices", false,
        Some("plan downgrade"), 9_000).await;

    let overrides = LicenseReadModel::list_overrides(&store, &TenantId::new("tenant_deny"))
        .await.unwrap();

    assert!(!overrides[0].allowed, "disabled override must have allowed = false");
...
async fn override_on_same_feature_replaces_previous() {
    let store = InMemoryStore::new();
    set_override(&store, "e1", "tenant_replace", "multi_provider", true,  None, 1_000).await;
    set_override(&store, "e2", "tenant_replace", "multi_provider", false, Some("revoked"), 2_000).await;
    ...
    assert!(!overrides[0].allowed, "latest override must take precedence");
...
async fn list_overrides_returns_all_features_for_tenant() {
    ...
    for (i, feature) in ["eval_matrices", "multi_provider", "governance_compliance"].iter().enumerate() {
        set_override(&store, &format!("e{i}"), "tenant_all",
            feature, true, None, 1_000 + i as u64).await;
    }

    let overrides = LicenseReadModel::list_overrides(&store, &TenantId::new("tenant_all"))
        .await.unwrap();

    assert_eq!(overrides.len(), 3, "all 3 feature overrides must be returned");
...
async fn list_overrides_scoped_to_tenant_only() {
    ...
    set_override(&store, "e1", "tenant_scope_a", "feature_x", true,  None, 1_000).await;
    set_override(&store, "e2", "tenant_scope_b", "feature_y", false, None, 1_000).await;
...
async fn license_is_scoped_to_individual_tenant() {
    ...
    activate_license(&store, "e1", "tenant_iso_a", "lic_a",
        ProductTier::TeamSelfHosted, 1_000, Some(10_000)).await;
    activate_license(&store, "e2", "tenant_iso_b", "lic_b",
        ProductTier::EnterpriseSelfHosted, 2_000, None).await;
    ...
    assert_eq!(rec_a.tier, ProductTier::TeamSelfHosted,       "tenant_a tier must be independent");
    assert_eq!(rec_b.tier, ProductTier::EnterpriseSelfHosted, "tenant_b tier must be independent");
...
async fn overrides_of_different_tenants_do_not_bleed() {
    ...
    set_override(&store, "e2", "tenant_q", "eval_matrices", false, None, 1_000).await;
...
async fn license_events_are_written_to_log() {
    ...
    activate_license(&store, "e1", "tenant_log", "lic_log",
        ProductTier::TeamSelfHosted, 1_000, Some(5_000)).await;
    set_override(&store, "e2", "tenant_log", "eval_matrices", true, None, 2_000).await;

    let all = store.read_stream(None, 100).await.unwrap();
    assert_eq!(all.len(), 2);

    assert!(matches!(&all[0].envelope.payload, RuntimeEvent::LicenseActivated(ev)
        if ev.tenant_id.as_str() == "tenant_log" && ev.valid_from_ms == 1_000));
    assert!(matches!(&all[1].envelope.payload, RuntimeEvent::EntitlementOverrideSet(ev)
