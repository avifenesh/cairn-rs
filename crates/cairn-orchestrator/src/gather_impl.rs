...
impl StandardGatherPhaseBuilder {
    pub fn with_retrieval(mut self, r: Arc<dyn RetrievalService>) -> Self { self.retrieval = Some(r); self }
    pub fn with_defaults(mut self, d: Arc<dyn DefaultsReadModel + Send + Sync>) -> Self { self.defaults = Some(d); self }
    pub fn with_checkpoints(mut self, c: Arc<dyn CheckpointReadModel + Send + Sync>) -> Self { self.checkpoints = Some(c); self }
    pub fn with_graph(mut self, g: Arc<dyn GraphQueryService>) -> Self { self.graph = Some(g); self }
    pub fn with_event_limit(mut self, n: usize) -> Self { self.event_limit = n; self }
    pub fn with_memory_limit(mut self, n: usize) -> Self { self.memory_limit = n; self }
    pub fn with_graph_limit(mut self, n: usize) -> Self { self.graph_limit = n; self }
...
impl GatherPhase for StandardGatherPhase {
    async fn gather(
        ...
        let recent_events = self.event_log
            .read_by_entity(
                &EntityRef::Run(ctx.run_id.clone()),
                None,
                self.event_limit,
            )
            .await
