...
mod tests {
    ...
    use cairn_runtime::{InMemoryServices, tasks::TaskService};
    ...
    async fn returns_task_state() {
        let svc = svc().await;
        svc.tasks.submit(&project(), TaskId::new("task_gt"), None, None, 0).await.unwrap();

        let tool = GetTaskTool::new(svc.store.clone());
        let res = tool.execute(&project(), serde_json::json!({ "task_id": "task_gt" })).await.unwrap();
    ...
    async fn not_found_is_permanent_error() {
        let svc = svc().await;
        let tool = GetTaskTool::new(svc.store.clone());
        let err = tool.execute(&project(), serde_json::json!({ "task_id": "nope" })).await.unwrap_err();
        assert!(matches!(err, ToolError::Permanent(_)));
    }

    #[test]
    fn tier_is_registered() { assert_eq!(GetTaskTool::new(Arc::new(cairn_store::InMemoryStore::new())).tier(), ToolTier::Registered); }
}
