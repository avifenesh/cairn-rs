...
use cairn_domain::{
    EventEnvelope, EventId, EventSource, ProjectKey, ScheduledTaskId, RuntimeEvent,
    events::ScheduledTaskCreated, policy::ExecutionClass,
};
use cairn_store::EventLog;
use serde_json::Value;
use super::{ToolError, ToolHandler, ToolResult, ToolTier};

pub struct ScheduleTaskTool { event_log: Arc<dyn EventLog + Send + Sync> }
...
impl ToolHandler for ScheduleTaskTool {
    ...
    fn description(&self) -> &str { "Create a recurring scheduled task with a cron expression." }
    ...
    async fn execute(&self, project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        let name = args.get("name").and_then(|v| v.as_str())
            .ok_or_else(|| ToolError::InvalidArgs { field: "name".into(), message: "required".into() })?
            .to_owned();
        let cron = args.get("cron_expression").and_then(|v| v.as_str())
            .ok_or_else(|| ToolError::InvalidArgs { field: "cron_expression".into(), message: "required".into() })?
            .to_owned();
        if name.trim().is_empty() {
            return Err(ToolError::InvalidArgs { field: "name".into(), message: "must not be empty".into() });
        }
...
mod tests {
    ...
    fn make_tool() -> ScheduleTaskTool { ScheduleTaskTool::new(Arc::new(InMemoryStore::new())) }

    #[test]
    fn name_tier() { assert_eq!(make_tool().name(), "schedule_task"); }

    #[tokio::test]
    async fn missing_name_is_invalid() {
        let err = make_tool().execute(&project(), serde_json::json!({"cron_expression":"0 9 * * 1"})).await.unwrap_err();
    ...
    async fn creates_scheduled_task() {
        ...
        assert!(matches!(&events[0].envelope.payload, RuntimeEvent::ScheduledTaskCreated(_)));
    }
}
