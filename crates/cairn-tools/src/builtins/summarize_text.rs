...
impl SummarizeTextTool {
    ...
    pub fn stub() -> Self {
        Self::new(Arc::new(NoopProvider), "none")
    }
}
impl Default for SummarizeTextTool {
    fn default() -> Self {
        Self::stub()
    }
}
...
mod tests {
    ...
    fn tier_is_registered() {
        assert_eq!(SummarizeTextTool::stub().tier(), ToolTier::Registered);
    }
    #[test]
    fn schema_has_required_text() {
        let s = SummarizeTextTool::stub().parameters_schema();
        let req = s["required"].as_array().unwrap();
        assert!(req.iter().any(|v| v.as_str() == Some("text")));
    }
    #[tokio::test]
    async fn stub_returns_summary() {
        let r = SummarizeTextTool::stub()
            .execute(&p(), serde_json::json!({"text":"hello world"}))
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn empty_text_err() {
        let err = SummarizeTextTool::stub()
            .execute(&p(), serde_json::json!({"text":"  "}))
            .await
            .unwrap_err();
        assert!(matches!(err, ToolError::InvalidArgs { .. }));
    }
    #[tokio::test]
    async fn missing_text_err() {
        let err = SummarizeTextTool::stub()
    ...
    #[tokio::test]
    async fn style_and_max_words_passed() {
        let r = SummarizeTextTool::stub()
            .execute(
                &p(),
                serde_json::json!({"text":"test","style":"bullet","max_words":50}),
            )
            .await
            .unwrap();
