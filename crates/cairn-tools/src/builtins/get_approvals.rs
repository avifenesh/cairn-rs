...
use cairn_domain::{ProjectKey, RunId, policy::ExecutionClass};
...
pub struct GetApprovalsTool { store: Arc<dyn ApprovalReadModel> }

impl GetApprovalsTool {
    pub fn new(store: Arc<dyn ApprovalReadModel>) -> Self { Self { store } }
}

#[async_trait]
impl ToolHandler for GetApprovalsTool {
    fn name(&self) -> &str { "get_approvals" }
    fn tier(&self) -> ToolTier { ToolTier::Registered }
    fn description(&self) -> &str { "List pending approvals for the current project, optionally filtered by run." }
    ...
    async fn execute(&self, project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        ...
        let records = ApprovalReadModel::list_pending(self.store.as_ref(), project, limit, 0).await
            .map_err(|e| ToolError::Transient(e.to_string()))?;

        // Optional run_id filter applied in-process (list_pending is project-scoped).
        let run_filter: Option<RunId> = args.get("run_id").and_then(|v| v.as_str())
            .map(RunId::new);

        let approvals: Vec<Value> = records.iter()
            .filter(|a| run_filter.as_ref().map_or(true, |rid| a.run_id.as_ref() == Some(rid)))
        ...
        Ok(ToolResult::ok(serde_json::json!({ "approvals": approvals, "total": total })))
    }
}
...
mod tests {
    ...
    async fn empty_store_returns_empty_list() {
        let result = GetApprovalsTool::new(Arc::new(InMemoryStore::new()))
            .execute(&project(), serde_json::json!({})).await.unwrap();
        assert_eq!(result.output["total"], 0);
    }

    #[tokio::test]
    async fn returns_pending_approvals() {
        use cairn_domain::{EventEnvelope, EventId, EventSource, ApprovalId, ApprovalRequested,
            RuntimeEvent, policy::ApprovalRequirement, RunId};
    ...
    async fn run_id_filter_works() {
        ...
        for (aid, rid) in [("a1","run_a"),("a2","run_b")] {
            store.append(&[EventEnvelope::for_runtime_event(
                EventId::new(format!("e_{aid}")), EventSource::Runtime,
                RuntimeEvent::ApprovalRequested(ApprovalRequested {
                    project: project(),
        ...
        let result = GetApprovalsTool::new(store)
            .execute(&project(), serde_json::json!({"run_id":"run_a"})).await.unwrap();
        assert_eq!(result.output["total"], 1);
