...
fn glob_segs(pat: &[&str], path: &[&str]) -> bool {
    match (pat.first(), path.first()) {
        ...
        (Some(&"**"), _) => {
            ...
            for i in 0..=path.len() {
                if glob_segs(&pat[1..], &path[i..]) { return true; }
            }
            false
        }
...
fn seg_match_chars(p: &[char], s: &[char]) -> bool {
    match (p.first(), s.first()) {
        ...
        (Some('*'), _) => {
            ...
            for i in 0..=s.len() {
                if seg_match_chars(&p[1..], &s[i..]) { return true; }
            }
            false
        }
...
impl Default for GlobFindTool { fn default() -> Self { Self } }

#[async_trait]
impl ToolHandler for GlobFindTool {
    fn name(&self) -> &str { "glob_find" }
    ...
    async fn execute(&self, _: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        let pattern = args["pattern"].as_str()
            .ok_or_else(|| ToolError::InvalidArgs { field:"pattern".into(), message:"required string".into() })?
            .trim();
        if pattern.is_empty() {
            return Err(ToolError::InvalidArgs { field:"pattern".into(), message:"must not be empty".into() });
        }
        let root = args["path"].as_str().unwrap_or(".");
        let max = args["max_results"].as_u64().unwrap_or(100).clamp(1, 500) as usize;

        if !std::path::Path::new(root).exists() {
            return Err(ToolError::InvalidArgs { field:"path".into(), message:format!("directory not found: {root}") });
        }
...
mod tests {
    ...
    #[test] fn tier_is_registered() { assert_eq!(GlobFindTool.tier(), ToolTier::Registered); }
    #[test] fn schema_requires_pattern() {
        let s = GlobFindTool.parameters_schema();
        assert!(s["required"].as_array().unwrap().iter().any(|v| v.as_str() == Some("pattern")));
    }
    #[test] fn glob_match_star_extension() {
        assert!(glob_match("*.rs",    "main.rs"));
        assert!(!glob_match("*.rs",   "main.ts"));
        assert!(glob_match("**/*.rs", "src/lib.rs"));
        assert!(glob_match("**/*.rs", "a/b/c/main.rs"));
    }
    #[test] fn glob_match_double_star() {
        assert!(glob_match("src/**", "src/a/b/c.rs"));
        assert!(glob_match("**",     "anything/at/all"));
    }
    #[test] fn glob_match_question_mark() {
        assert!(glob_match("file?.rs", "file1.rs"));
        assert!(!glob_match("file?.rs","file12.rs"));
    }
    #[tokio::test] async fn empty_pattern_err() {
        let err = GlobFindTool.execute(&p(), serde_json::json!({"pattern":""})).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidArgs { .. }));
    }
    #[tokio::test] async fn missing_pattern_err() {
        let err = GlobFindTool.execute(&p(), serde_json::json!({})).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidArgs { .. }));
    }
    #[tokio::test] async fn finds_toml_files() {
        // Should find at least the workspace Cargo.toml
        let r = GlobFindTool.execute(&p(), serde_json::json!({"pattern":"*.toml","path":"."})).await;
        match r {
            Ok(res) => {
