...
use cairn_domain::{
    ApprovalId, CheckpointId, EvalRunId, IngestJobId, MailboxMessageId, ProjectKey,
    PromptAssetId, PromptReleaseId, PromptVersionId, RunId, SessionId, SignalId, TaskId,
    ToolInvocationId, policy::ExecutionClass,
};
...
pub struct SearchEventsTool { event_log: Arc<dyn EventLog + Send + Sync> }

impl SearchEventsTool {
    pub fn new(event_log: Arc<dyn EventLog + Send + Sync>) -> Self { Self { event_log } }
}
...
impl ToolHandler for SearchEventsTool {
    fn name(&self) -> &str { "search_events" }
    fn tier(&self) -> ToolTier { ToolTier::Registered }
    fn description(&self) -> &str { "Query the event log for recent events, optionally scoped to one entity." }
    ...
    async fn execute(&self, _project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        let limit = args.get("limit").and_then(|v| v.as_u64())
            .unwrap_or(DEFAULT_LIMIT as u64).min(MAX_LIMIT as u64) as usize;
        let after = args.get("after").and_then(|v| v.as_u64()).map(EventPosition);
        ...
        let events = if let Some(entity_id) = args.get("entity_id").and_then(|v| v.as_str()) {
            ...
            let entity_ref = parse_entity_ref(entity_type, entity_id)
                .ok_or_else(|| ToolError::InvalidArgs {
                    field: "entity_type".into(),
                    message: format!("unknown entity type: '{entity_type}'"),
                })?;
            self.event_log.read_by_entity(&entity_ref, after, limit).await
        ...
        let event_list: Vec<Value> = events.iter().map(|e| serde_json::json!({
        ...
        Ok(ToolResult::ok(serde_json::json!({ "events": event_list, "total": total })))
    }
}
...
mod tests {
    ...
    fn make_tool() -> SearchEventsTool { SearchEventsTool::new(Arc::new(InMemoryStore::new())) }

    #[test]
    fn name_tier() { assert_eq!(make_tool().name(), "search_events"); }

    #[tokio::test]
    async fn empty_log_returns_empty_list() {
        let result = make_tool().execute(&project(), serde_json::json!({})).await.unwrap();
    ...
    async fn unknown_entity_type_is_invalid() {
        let err = make_tool()
            .execute(&project(), serde_json::json!({"entity_id":"r","entity_type":"widget"}))
            .await.unwrap_err();
    ...
    async fn returns_events_from_stream() {
        ...
        store.append(&[EventEnvelope::for_runtime_event(
            EventId::new("e1"), EventSource::Runtime,
            RuntimeEvent::RunCreated(RunCreated {
                project: project(), run_id: RunId::new("run_1"),
        ...
        let result = tool.execute(&project(), serde_json::json!({})).await.unwrap();
