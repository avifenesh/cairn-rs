...
#[derive(Clone)]
struct Entry { value: Value, expires_at_ms: u64 }
...
impl Default for ScratchPadTool {
    fn default() -> Self { Self { store: Arc::new(Mutex::new(HashMap::new())) } }
}
...
impl ToolHandler for ScratchPadTool {
    ...
    async fn execute(&self, _project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        let action = args.get("action").and_then(|v| v.as_str())
            .ok_or_else(|| ToolError::InvalidArgs { field: "action".into(), message: "required".into() })?;
        let now = now_ms();
        ...
        match action {
            "write" => {
                ...
                let value = args.get("value").cloned()
                    .ok_or_else(|| ToolError::InvalidArgs { field: "value".into(), message: "required for write".into() })?;
                let ttl_ms = args.get("ttl_minutes").and_then(|v| v.as_u64())
                    .unwrap_or(DEFAULT_TTL_MINUTES) * 60_000;
                let expires_at_ms = now + ttl_ms;
                self.store.lock().unwrap().insert(key.clone(), Entry { value, expires_at_ms });
            ...
            "read" => {
                ...
                match map.get(&key) {
                    ...
                    Some(_) => {
                        map.remove(&key);
                        Ok(ToolResult::ok(serde_json::json!({ "key": key, "found": false, "reason": "expired" })))
                    }
                    None => Ok(ToolResult::ok(serde_json::json!({ "key": key, "found": false }))),
                }
            }
            "list" => {
                ...
                Ok(ToolResult::ok(serde_json::json!({ "keys": entries, "total": total })))
            }
            "delete" => {
                let key = require_key(&args)?;
                let removed = self.store.lock().unwrap().remove(&key).is_some();
                Ok(ToolResult::ok(serde_json::json!({ "key": key, "deleted": removed })))
            }
...
fn require_key(args: &Value) -> Result<String, ToolError> {
    let k = args.get("key").and_then(|v| v.as_str())
        .ok_or_else(|| ToolError::InvalidArgs { field: "key".into(), message: "required".into() })?;
    if k.trim().is_empty() {
        return Err(ToolError::InvalidArgs { field: "key".into(), message: "must not be empty".into() });
    }
    Ok(k.to_owned())
}
...
mod tests {
    ...
    #[tokio::test] async fn write_then_read() {
        let t = ScratchPadTool::new();
        t.execute(&project(), serde_json::json!({"action":"write","key":"k","value":{"x":1}})).await.unwrap();
        let r = t.execute(&project(), serde_json::json!({"action":"read","key":"k"})).await.unwrap();
        assert_eq!(r.output["found"], true);
        assert_eq!(r.output["value"]["x"], 1);
    }

    #[tokio::test] async fn read_missing_key_returns_not_found() {
        let t = ScratchPadTool::new();
        let r = t.execute(&project(), serde_json::json!({"action":"read","key":"no"})).await.unwrap();
        assert_eq!(r.output["found"], false);
    }

    #[tokio::test] async fn list_shows_active_keys() {
        let t = ScratchPadTool::new();
        t.execute(&project(), serde_json::json!({"action":"write","key":"a","value":1})).await.unwrap();
        t.execute(&project(), serde_json::json!({"action":"write","key":"b","value":2})).await.unwrap();
        let r = t.execute(&project(), serde_json::json!({"action":"list"})).await.unwrap();
        assert_eq!(r.output["total"], 2);
    }

    #[tokio::test] async fn delete_removes_key() {
        let t = ScratchPadTool::new();
        t.execute(&project(), serde_json::json!({"action":"write","key":"del","value":"v"})).await.unwrap();
        let r = t.execute(&project(), serde_json::json!({"action":"delete","key":"del"})).await.unwrap();
        assert_eq!(r.output["deleted"], true);
        let r2 = t.execute(&project(), serde_json::json!({"action":"read","key":"del"})).await.unwrap();
        assert_eq!(r2.output["found"], false);
    }

    #[tokio::test] async fn delete_missing_key_returns_false() {
        let t = ScratchPadTool::new();
        let r = t.execute(&project(), serde_json::json!({"action":"delete","key":"ghost"})).await.unwrap();
        assert_eq!(r.output["deleted"], false);
    }

    #[tokio::test] async fn unknown_action_is_invalid() {
        let err = ScratchPadTool::new()
            .execute(&project(), serde_json::json!({"action":"nuke"})).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidArgs { .. }));
    }

    #[tokio::test] async fn write_returns_expires_at() {
        let t = ScratchPadTool::new();
        let r = t.execute(&project(), serde_json::json!({"action":"write","key":"k","value":1,"ttl_minutes":60})).await.unwrap();
        let exp = r.output["expires_at_ms"].as_u64().unwrap();
        let now = now_ms();
        assert!(exp > now + 50 * 60_000, "expires_at must be ~60 min from now");
    }
}
