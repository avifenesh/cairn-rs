...
fn tenant() -> TenantId {
    TenantId::new("t_retention")
}
fn project() -> ProjectKey {
    ProjectKey::new("t_retention", "w_retention", "p_retention")
}
...
async fn setup() -> (Arc<InMemoryStore>, RetentionServiceImpl<InMemoryStore>) {
    ...
    TenantServiceImpl::new(store.clone())
        .create(tenant(), "Retention Tenant".to_owned())
        .await
        .unwrap();
    WorkspaceServiceImpl::new(store.clone())
        .create(
            tenant(),
            WorkspaceId::new("w_retention"),
            "Workspace".to_owned(),
        )
        .await
        .unwrap();
...
async fn update_policy_replaces_previous() {
    ...
    assert_ne!(
        updated.policy_id, first_id,
        "updated policy must have a new policy_id"
    );
...
async fn apply_retention_enforces_retention_window() {
    ...
    let before = store
        .read_by_entity(&EntityRef::Session(session_id.clone()), None, 100)
        .await
        .unwrap();
    ...
    assert!(
        result.events_pruned >= 7,
        "at least 7 events must be pruned (10 - 3); got: {}",
    ...
    assert!(
        after.len() <= 3,
        "at most 3 events must remain; got: {}",
        after.len()
    );
...
async fn set_policy_for_missing_tenant_returns_not_found() {
    ...
    let err = svc
        .set_policy(TenantId::new("ghost"), 30, 90, 100)
        .await
        .unwrap_err();
    assert!(
        matches!(
...
async fn tenant_policies_are_independent() {
    ...
    tenant_svc
        .create(t1.clone(), "Tenant A".to_owned())
        .await
        .unwrap();
    tenant_svc
        .create(t2.clone(), "Tenant B".to_owned())
        .await
        .unwrap();
    let svc = RetentionServiceImpl::new(store.clone());
    ...
    assert_eq!(
        p2_after.full_history_days, 7,
        "updating t1 must not affect t2"
    );
...
async fn zero_max_events_per_entity_skips_pruning() {
    ...
    assert_eq!(
        ...
        "max=0 must skip pruning (unlimited)"
    );
}
