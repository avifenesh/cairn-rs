...
async fn ingest_three_docs() -> InMemoryRetrieval {
    ...
    let pipeline = IngestPipeline::new(store.clone(), ParagraphChunker { max_chunk_size: 120 });
...
async fn deep_search_finds_relevant_chunks_across_documents() {
    ...
    let response = search.search(DeepSearchRequest {
        project: project(),
    ...
    assert!(!response.merged_results.is_empty(), "deep search must return results for a matching query");

    // Results must relate to ownership/data-race topic.
    let texts: Vec<_> = response.merged_results.iter().map(|r| r.chunk.text.as_str()).collect();
    let relevant = texts.iter().any(|t| t.contains("ownership") || t.contains("data race") || t.contains("borrow"));
    assert!(relevant, "at least one result must mention ownership or data races");
}
...
async fn deep_search_with_max_hops_2_performs_at_least_one_hop() {
    ...
    assert_eq!(response.hops.len(), 2, "tight quality gate forces 2 hops (max_hops=2)");

    // Each hop must have a sub_query and latency recorded.
    for hop in &response.hops {
        assert!(!hop.sub_query.is_empty(), "hop {} must have a sub_query", hop.hop_number);
...
async fn deep_search_response_reports_hops_and_latency() {
    ...
    assert!(!response.hops.is_empty(), "hops diagnostics must be non-empty");

    // Hop 0 always exists.
    let hop0 = &response.hops[0];
    assert_eq!(hop0.hop_number, 0, "first hop must be numbered 0");
    assert!(!hop0.sub_query.is_empty(), "hop 0 must record its sub_query");

    // Outcome is one of the valid variants.
    assert!(
        matches!(hop0.outcome, HopOutcome::Sufficient | HopOutcome::NeedsExpansion | HopOutcome::Exhausted),
    ...
    assert!(!response.merged_results.is_empty() || response.hops[0].outcome == HopOutcome::Exhausted);
}
...
async fn deep_search_on_empty_project_exhausts_on_first_hop() {
    ...
    assert_eq!(response.hops.len(), 1, "empty project must exhaust on the first hop");
    assert_eq!(response.hops[0].outcome, HopOutcome::Exhausted);
    assert!(response.merged_results.is_empty(), "no results for empty project");
}
...
async fn loose_quality_gate_stops_search_after_first_hop() {
    ...
    assert_eq!(response.hops.len(), 1, "loose gate must stop after finding results on hop 0");
    assert_eq!(response.hops[0].outcome, HopOutcome::Sufficient);
    assert!(!response.merged_results.is_empty(), "results must be present on early stop");
}
...
async fn merged_results_contain_no_duplicate_chunk_ids() {
    ...
    for result in &response.merged_results {
        ...
        assert!(seen.insert(id.clone()), "duplicate chunk ID in merged_results: {id}");
    }
}
