...
use cairn_graph::queries::{GraphQuery, GraphQueryError, GraphQueryService, Subgraph, TraversalDirection};
...
fn project() -> ProjectKey { ProjectKey::new("t_gx", "ws_gx", "proj_gx") }
...
impl TestGraph {
    fn new() -> Self { Self { neighbors: Mutex::new(HashMap::new()) } }
...
impl GraphQueryService for TestGraph {
    ...
    async fn neighbors(
        &self, node_id: &str, _: Option<EdgeKind>, _: TraversalDirection, _: usize,
    ) -> Result<Vec<(GraphEdge, GraphNode)>, GraphQueryError> {
        Ok(self.neighbors.lock().unwrap().get(node_id).cloned().unwrap_or_default())
    }
...
struct TermInjector { terms: Vec<String> }
...
async fn setup_retrieval() -> InMemoryRetrieval {
    ...
    let pipeline = IngestPipeline::new(store.clone(), ParagraphChunker { max_chunk_size: 200 });

    pipeline.submit(ingest_req("doc_a",
    ...
    pipeline.submit(ingest_req("doc_b",
    ...
    pipeline.submit(ingest_req("doc_c",
...
async fn expansion_follows_all_supported_edge_kinds() {
    ...
    assert_eq!(expansions.len(), 4, "all four edge kinds must yield expansions");
}
...
async fn expansion_terms_appear_in_hop_sub_queries() {
    ...
    let search = IterativeDeepSearch::new(retrieval)
        ...
        .with_quality_gate(QualityGateConfig { min_score_threshold: 0.99, min_results: 100 });
...
async fn max_expansions_limits_graph_expansion_output() {
    ...
    assert!(expansions.len() <= 3, "max_expansions=3 must cap at 3, got {}", expansions.len());
}
...
async fn expansion_deduplicates_related_nodes() {
    ...
    let expansions = hook.expand("test", &results).await;
    let shared_count = expansions.iter().filter(|e| e.as_str() == "doc_shared").count();
...
async fn mock_expansion_injects_term_into_hop_1_sub_query() {
    let retrieval = setup_retrieval().await;
    let search = IterativeDeepSearch::new(retrieval)
        .with_graph_hook(TermInjector { terms: vec!["fearless".to_owned()] })
        .with_quality_gate(QualityGateConfig { min_score_threshold: 0.99, min_results: 100 });
...
async fn max_hops_bounds_number_of_hops() {
    let retrieval = setup_retrieval().await;
    let search = IterativeDeepSearch::new(retrieval)
        .with_graph_hook(TermInjector { terms: vec!["extra".to_owned()] })
        .with_quality_gate(QualityGateConfig { min_score_threshold: 0.99, min_results: 100 });
    ...
    for max_hops in [1u32, 2, 3] {
        ...
        let response = s.search(DeepSearchRequest {
            project: project(),
            query_text: "rust".to_owned(),
            max_hops,
            per_hop_limit: 5,
...
async fn empty_graph_produces_no_expansion_terms() {
    ...
    assert!(expansions.is_empty(), "empty graph must produce no expansion terms");
}
