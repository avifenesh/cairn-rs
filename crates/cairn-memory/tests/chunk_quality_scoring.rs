...
fn chunk_quality_scoring_formula_long_alphanumeric_scores_high() {
    ...
    assert!(q > 0.7, "long alphanumeric-rich text must score > 0.7, got {q:.4}");
}
...
fn chunk_quality_scoring_formula_short_special_chars_scores_low() {
    ...
    assert!(q < 0.3, "short special-char text (no provenance) must score < 0.3, got {q:.4}");
}
...
fn chunk_quality_scoring_formula_no_provenance_lowers_score() {
    ...
    assert!(with_prov > without_prov, "provenance=true must score higher than provenance=false");
}
...
async fn chunk_quality_scoring_poor_quality_chunk_scores_low() {
    ...
    assert!(q < 0.3, "formula must give < 0.3 for poor chunk, got {q:.4}");

    // Insert chunk with pre-computed quality directly.
    store.insert_chunks(&[ChunkRecord {
...
async fn chunk_quality_scoring_appears_in_scoring_dimensions() {
    ...
    pipeline
        .submit(IngestRequest {
            ...
            source_type: SourceType::PlainText,
            project: project(),
            content: "Quality scoring dimension tracking test content here unique words.".to_owned(),
            tags: vec![],
            corpus_id: None,
            bundle_source_id: None,
            import_id: None,
...
async fn chunk_quality_scoring_high_quality_outscores_low_quality_in_retrieval() {
    let store = Arc::new(InMemoryDocumentStore::new());
    let pipeline = IngestPipeline::new(store.clone(), ParagraphChunker { max_chunk_size: 2000 });
    let retrieval = InMemoryRetrieval::new(store.clone());
