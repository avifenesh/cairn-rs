...
impl ToolHandler for ConcreteMemorySearchTool {
    ...
    async fn execute(&self, project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        ...
        let mode = match args.get("mode").and_then(|v| v.as_str()).unwrap_or("lexical") {
        ...
        match self.retrieval.query(query).await {
            Ok(resp) => {
                let results: Vec<Value> = resp.results.into_iter().map(|r| serde_json::json!({
...
impl ToolHandler for ConcreteMemoryStoreTool {
    ...
    async fn execute(&self, project: &ProjectKey, args: Value) -> Result<ToolResult, ToolError> {
        ...
        let source_label = args.get("source_id")
            .and_then(|v| v.as_str())
            .unwrap_or("agent")
            .to_owned();
        ...
        self.ingest.submit(IngestRequest {
        ...
        }).await.map_err(|e| ToolError::Transient(format!("ingest failed: {e}")))?;
...
mod tests {
    ...
    fn make_ingest() -> (Arc<InMemoryDocumentStore>, Arc<IngestPipeline<Arc<InMemoryDocumentStore>, ParagraphChunker>>) {
        let store = Arc::new(InMemoryDocumentStore::new());
        let pipeline = Arc::new(IngestPipeline::new(store.clone(), ParagraphChunker::default()));
        (store, pipeline)
    }
    ...
    async fn search_finds_ingested_content() {
        ...
        assert!(text.contains("cairn") || text.contains("Rust"),
    ...
    async fn store_returns_document_id() {
        ...
        assert!(result.output["document_id"].as_str().unwrap().starts_with("mem_"));
    }
    ...
    async fn stored_content_is_searchable() {
        ...
        store_tool.execute(&project(), serde_json::json!({
            "content": "cairn-rs uses lexical search for memory retrieval"
        })).await.unwrap();

        let result = search_tool.execute(&project(), serde_json::json!({
            "query": "cairn memory retrieval"
        })).await.unwrap();

        let total = result.output["total"].as_u64().unwrap();
        assert!(total > 0, "just-stored content must be immediately searchable");
    }
    ...
    async fn store_rejects_empty_content() {
        let (_, pipeline) = make_ingest();
        let tool = ConcreteMemoryStoreTool::new(pipeline);
        let err = tool.execute(&project(), serde_json::json!({ "content": "  " })).await.unwrap_err();
        assert!(matches!(err, ToolError::InvalidArgs { .. }));
    }
    ...
    async fn registry_dispatches_memory_store() {
        let (_, pipeline) = make_ingest();
        let retrieval = Arc::new(InMemoryRetrieval::new(Arc::new(InMemoryDocumentStore::new())))
