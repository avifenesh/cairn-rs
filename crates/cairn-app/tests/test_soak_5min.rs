//! 5-minute real-LLM soak test (step 3 of 3 reality-check tests).
//!
...
//! runs** against it for **300 real seconds**. Every 15 seconds samples
...
//! ```
//!
//! Expect ~5 min + ~10s for cairn-app boot.
//!
//! # CI posture
//!
...
//! - 30-min soak (#174) and 1-hr soak (#175).
//! - Tight empirical bounds on RSS/fd growth — the first-iteration values
//!   here are intentionally loose; tighten once we have baseline data
...
/// Number of concurrent soak workers. Kept small so 5 min × 10 s cadence
/// stays well under any reasonable rate budget (~90 total calls).
const WORKERS: usize = 3;

/// Total soak duration. Hard-coded — follow-up PRs own 30 min and 1 hr.
const SOAK_DURATION: Duration = Duration::from_secs(300);

/// Interval between successive orchestrations within one worker. Spaces
/// requests so the free tier is never stressed.
const WORKER_INTERVAL: Duration = Duration::from_secs(10);

/// Interval between metric samples. 15 s × 5 min = 20 samples, enough
/// resolution to see leak trajectories without spam.
const SAMPLE_INTERVAL: Duration = Duration::from_secs(15);

/// Loose RSS growth bound — first-iteration value. Real leak is usually
/// 5-10x, so 50% has plenty of headroom against noise while still
...
/// things: one-time startup fd cost and actual leak growth. At a small
/// baseline (~16 fds), a fixed +4-6 fd startup cost trips a 20-30 %
/// relative bound spuriously even though sample traces show zero
/// growth after warmup. See commit body for the 2026-04-22 30min
/// trace.
///
/// Warmup is defined as a `Duration` so the semantics stay stable if
/// `SAMPLE_INTERVAL` changes; steady-state samples are those whose
/// `elapsed_s` is at least `FD_WARMUP_DURATION`. The 5min test keeps
/// a smaller warmup window (60 s) since the whole run is only 5 min.
const FD_WARMUP_DURATION: Duration = Duration::from_secs(60);
...
#[tokio::test]
#[ignore = "real-LLM soak: requires ~/.cairn-secrets/openrouter.key and ~5 min"]
async fn cairn_app_sustains_5min_real_llm_soak() {
    ...
    //    init opens persistent fds) then assert that the spread across
    //    the steady-state window is bounded. This directly tests
    //    "no growth in the steady state" — a baseline→end % bound
    //    conflates startup cost with leak growth and is brittle at
    //    small baselines.
