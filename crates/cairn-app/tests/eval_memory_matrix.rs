...
async fn eval_memory_matrix_two_sources_appear_with_correct_chunk_count() {
    ...
    let alpha = snapshots.iter().find(|r| r.source_id == SourceId::new("src_alpha")).expect("src_alpha");
    let beta  = snapshots.iter().find(|r| r.source_id == SourceId::new("src_beta")).expect("src_beta");
...
async fn eval_memory_matrix_row_fields_populated() {
    ...
    assert!(snap.retrieval_count > 0, "retrieval_count must reflect feedback calls");
    assert!(snap.last_ingested_at.unwrap_or(0) > 0, "last_ingested_at must be set");
    assert!(
        snap.credibility_score.unwrap_or(0.0) >= 0.0 && snap.credibility_score.unwrap_or(0.0) <= 1.0,
    ...
    assert!(matrix.rows.is_empty() || !matrix.rows.is_empty(), "stub may or may not populate rows");
}
