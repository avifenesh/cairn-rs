...
use cairn_api::onboarding::{
    compute_progress, create_onboarding_checklist, materialize_template, StarterTemplateRegistry,
};
use cairn_domain::onboarding::{OnboardingFlowState, StarterTemplateCategory};
use cairn_domain::ProjectKey;
use cairn_domain::{ProjectId, TenantId, WorkspaceId};
use cairn_runtime::projects::ProjectService;
use cairn_runtime::services::{ProjectServiceImpl, TenantServiceImpl, WorkspaceServiceImpl};
...
fn list_available_templates() {
    ...
    assert_eq!(
        templates.len(),
        3,
        "V1 registry must ship exactly 3 starter templates"
    );
    ...
    for t in templates {
        ...
        assert!(
            ...
            "template '{}' must have at least one prompt asset or policy preset",
            t.id
        );
    }
}
...
async fn materialize_template_creates_provenance_with_correct_scope() {
    ...
    for asset in &provenance.materialized_assets {
        assert!(
            ...
            "asset_id must be scoped to the project; got: {}",
            asset.asset_id
        );
        assert!(
            !asset.diverged,
            "freshly materialized assets must not be marked diverged"
        );
        assert!(
            asset
                .source_template_ref
                .starts_with("knowledge-assistant:"),
...
fn create_checklist_all_steps_initially_incomplete() {
    ...
    assert_eq!(
        checklist.template_id.as_deref(),
    ...
    assert_eq!(
        checklist.steps.len(),
        7,
        "V1 checklist must have exactly 7 steps"
    );
    ...
    assert_eq!(
        progress.len(),
        7,
        "progress must have one entry per checklist step"
    );

    for step in &progress {
        assert!(
            !step.completed,
            "step '{}' must be incomplete on a fresh checklist",
            step.step_id
        );
        assert!(
            step.completed_at.is_none(),
            "step '{}' must have no completed_at on a fresh checklist",
            step.step_id
        );
        assert!(
            !step.label.is_empty(),
            "step '{}' must have a non-empty label",
            step.step_id
        );
...
fn mark_step_complete_reflects_in_progress() {
    ...
    let create_before = before
        .iter()
        .find(|p| p.step_id == "create_project")
        .unwrap();
    assert!(
        !create_before.completed,
        "create_project must start incomplete"
    );
    ...
    let step2 = checklist
        .steps
        .iter_mut()
        .find(|s| s.step_id == "configure_provider")
        .unwrap();
    ...
    assert_eq!(
        completed_count, 2,
        "exactly 2 steps must be marked complete"
    );

    let configure_after = after2
        .iter()
        .find(|p| p.step_id == "configure_provider")
        .unwrap();
    assert!(configure_after.completed);
    assert!(
        configure_after.required,
        "configure_provider is a required step"
    );
}
...
fn all_templates_materialize_expected_asset_counts() {
    ...
    for (template_id, expected_count) in expected {
        ...
        let provenance =
            materialize_template(template, &tenant_id, &workspace_id, &project_id, now);
        assert_eq!(
            provenance.materialized_assets.len(),
            expected_count,
            "template '{}' must materialize {} assets",
            template_id,
            expected_count
        );
    }
...
fn batch_import_reconciliation_counts() {
    use cairn_api::onboarding::{reconcile_prompt_imports, PromptImportItem};
    ...
    assert_eq!(
        report.created_count, 2,
        "two new keys: 'new.prompt' and 'imp_sys'"
    );
    assert_eq!(
        report.skipped_count, 1,
        "system.prompt with hash_v1 is reused"
    );
