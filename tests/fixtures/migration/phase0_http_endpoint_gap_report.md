...
| `GET /v1/memories/search?q=test&limit=10` | `dedicated_endpoint_trait_present` | `MemoryEndpoints::search` exists in `memory_api.rs`, and the current `MemoryItem` response shape now carries the preserved `source`, `confidence`, and ISO-string `createdAt` fields exercised by the fixture-alignment tests. | `keep_contract_stable` |
