...
| `memory_proposed` | `supported_via_dedicated_builder` | Covered by `build_memory_proposed_frame(...)` in `sse_payloads.rs`; wired to the SSE broadcast channel via `SseMemoryProposalHook` in `cairn-app`. | `keep` |
