...
| `assistant_end` | `supported_via_dedicated_builder` | Covered by `build_streaming_sse_frame(StreamingOutput::AssistantEnd, ...)`, and `StreamingOutput::AssistantEnd` now carries the fully assembled reply text needed for the preserved `messageText` payload. | `keep` |
...
| `memory_proposed` | `supported_via_dedicated_builder` | Covered by `build_memory_proposed_frame(...)` in `sse_payloads.rs`; wired to the SSE broadcast channel via `SseMemoryProposalHook` in `cairn-app`, and now emitted through the live preserved memory create path as well. | `keep` |
