...
| `GET /v1/memories/search?q=test&limit=10` | `MemoryEndpoints::search` present in `memory_api.rs` | `Worker 6` + `Worker 8` | Retrieval/search boundary is explicit now, and `cairn-app` composes the preserved `/v1/memories*` routes through the active `MemoryApiImpl`; the remaining memory work is durable backing, not HTTP/SSE seam wiring. |
...
| `assistant_end` | dedicated assistant-streaming builder present | `Worker 8` + `Worker 7` | Final assistant message event is builder-backed directly from `StreamingOutput::AssistantEnd`, which now carries the fully assembled reply text for the preserved `{ taskId, messageText }` contract. |
...
| `memory_proposed` | dedicated non-runtime builder present; wired to SSE broadcast via `SseMemoryProposalHook` and the active `MemoryApiImpl` path | `Worker 8` + `Worker 6` | `build_memory_proposed_frame(...)` is wired to the SSE broadcast channel in `cairn-app`, and preserved memory creates now flow through that same hook via the live `MemoryApiImpl`; preserve the composed path as-is. |
